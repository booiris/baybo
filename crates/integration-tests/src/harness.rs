...
impl AgentTestHarnessBuilder {
    ...
    pub fn build(self) -> AgentTestHarness {
        ...
        let goal_store: Arc<dyn baybo_store::GoalStore> =
            Arc::new(baybo_goal::test_support::MemoryGoalStore::new());
        for (tool, manifest) in baybo_goal::tools::agent_tools(goal_store.clone()) {
            tool_registry.register(tool, manifest);
        }
        let goal_service = Arc::new(baybo_goal::GoalService::new(goal_store.clone()));
