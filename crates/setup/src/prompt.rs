...
pub trait Prompter: Send {
    ...
    /// Multi-select prompt. `initial[i]` seeds option `i`'s checked state
    /// (missing entries default to unchecked). Returns the checked indices
    /// in ascending order — empty when nothing is checked.
    fn multi_select(
        &mut self,
        label: &str,
        options: &[&str],
