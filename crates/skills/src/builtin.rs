...
//! useful defaults the moment the gateway boots — no `aura skills
...
const AURA_CLI_SKILL_MD: &str = include_str!("builtin/aura-cli/SKILL.md");

/// Template token in built-in SKILL.md bodies that gets substituted
/// with the absolute path of the running `aura` binary at register
/// time. The skill body uses `{{AURA_BIN}} status --live` instead of
/// bare `aura status --live` so the agent's `Bash` tool can invoke
/// the CLI even when `aura` isn't on `$PATH` (the common case in dev
/// builds and in detached deployments).
const AURA_BIN_TOKEN: &str = "{{AURA_BIN}}";
...
pub(crate) fn all() -> Vec<SkillDefinition> {
    ...
    let raw = [("aura-cli", AURA_CLI_SKILL_MD)];
    raw.into_iter()
...
/// [`aura_workspace::paths::BIN_NAME`] when `current_exe()` errors
    ...
    match std::env::current_exe() {
        ...
        Err(e) => {
            tracing::warn!(
                ...
                aura_workspace::paths::BIN_NAME,
            );
            aura_workspace::paths::BIN_NAME.to_string()
        }
    }
}
...
/// produce. Mirrors `aura_tools::builtin::bash::sh_quote`; copied
/// here so `aura-skills` doesn't gain a dependency on `aura-tools`.
...
fn parse(name: &str, md: &str, bin: &str) -> Option<SkillDefinition> {
    match parse_skill_md(md, name) {
        Ok(mut skill) => {
            skill.prompt_template = skill.prompt_template.replace(AURA_BIN_TOKEN, bin);
...
mod tests {
    ...
    #[test]
    fn aura_bin_token_is_substituted_in_prompt_template() {
        let bin = sh_quote("/fake/bin/aura");
        let skill = parse(
            "aura-cli",
            "---\nname: aura-cli\ndescription: t\n---\nRun {{AURA_BIN}} status --live.",
            &bin,
        )
        .expect("test skill parses");
        assert!(
            skill.prompt_template.contains("'/fake/bin/aura'"),
            "expected absolute bin path in body, got: {}",
            skill.prompt_template
        );
        assert!(
            !skill.prompt_template.contains(AURA_BIN_TOKEN),
