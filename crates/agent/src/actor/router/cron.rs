...
use super::user_input::resolve_spawn_llm;
use super::{ActorSpawner, Router, SpawnLlmChoice};
...
impl Router {
    ...
    async fn run_conversation_fire(&self, session: Session, trigger: AgentMessage) {
        ...
        let routed = self
            ...
            .route_or_spawn(&session_id, trigger, || {
                ...
                actor_spawner(session, SpawnLlmChoice::default(), response_tx, actor_token)
            })
            .await;
        if !routed {
    ...
    fn cron_result_delivery(&self) -> CronResultDelivery {
        CronResultDelivery {
            ...
            agent_profile_store: Arc::clone(&self.agent_profile_store),
        }
    }
...
pub(super) struct CronResultDelivery {
    ...
    /// Live agent-profile reads for [`resolve_spawn_llm`] — a re-hydrated
    /// origin conversation must resolve its LLM pin + reasoning effort the
    /// same way a user message would.
...
impl CronResultDelivery {
    ...
    async fn deliver(&self, origin_session_id: Option<SessionId>, result: PendingCronResult) {
        ...
        let parent_token = self.actor_parent_token.clone();
        let actor_spawner = self.actor_spawner.as_ref();
        let llm_choice = resolve_spawn_llm(&self.agent_profile_store, &origin).await;
        let delivered = self
            .supervisor
            .route_or_spawn(
                &origin_id,
                ...
                || {
                    let actor_token = parent_token.child_token();
                    actor_spawner(origin, llm_choice, response_tx, actor_token)
                },
            )
            .await;
        if !delivered {
