...
impl Router {
    ...
    /// the rare cold-spawn branch. The profile lookup is async, so it's
    /// resolved here (this fn is already async) rather than inside the sync
    /// `route_or_spawn` closure.
