// Baybo provider name -> OpenRouter catalog prefix. The single source of
...
// `(baybo_provider_name, openrouter_prefix)`. Providers absent here don't
...
// OpenRouter catalog prefix for an Baybo provider, or `None` when the
