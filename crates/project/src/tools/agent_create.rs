...
impl Tool for ProjectAgentCreateTool {
    ...
    fn description(&self) -> String {
        format!(
            r#"Add a new agent to this project's team, so there is somebody to assign work that nobody here can do. Returns its `@handle`, which is derived from the name and never changes.
    ...
    fn parameters_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                "name": {
                    "type": "string",
                    "description": "Display name, e.g. \"Test Engineer\". Its `@handle` is derived from this.",
                },
                "role": {
                    "type": "string",
