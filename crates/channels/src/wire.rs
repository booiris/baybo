...
///
/// `2` — Register frame no longer carries `session_id`; `Subscribe` /
/// `Unsubscribe` frames express per-session interest; `Reset` frame
/// signals "client should re-fetch history"; `Message` gains a `role`
/// discriminator so server echo of inbound user messages is
/// disambiguated from agent replies.
///
/// `Subscribe { since_ordinal }` and `Message { ordinal }` were added
/// inside `2` after the fact — both are `Option`-shaped with serde
/// `default + skip_serializing_if = "Option::is_none"`, so a `2`-era
/// peer that omits them stays wire-compatible: producers without the
/// field round-trip to consumers without the field, and a new producer
/// just gets the catch-up benefit when paired with a new consumer.
