...
///
/// A new child node is created under `from_node` to serve as the fork root,
/// and a `ForkRecord` is appended to the trace. The `active_leaf` is updated
/// to point to the new fork root.
///
