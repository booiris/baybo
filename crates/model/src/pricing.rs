...
//! replace it. Lives in `baybo-model` so `baybo-config` (persistence)
//! and `baybo-llm` (consumption) can share one struct without taking a
...
/// `baybo_llm::ModelPricing`: integer micro-USD per **1 million tokens**.
