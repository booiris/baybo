//! libsql implementation of [`TaskStore`] — the per-session planning
...
fn try_task_from_row(row: &libsql::Row) -> Option<Task> {
...
impl TaskStore for LibsqlTaskStore {
    async fn create(&self, session_id: &SessionId, task: &Task) -> Result<()> {
        ...
        conn.execute(
            ...
            libsql::params![
                ...
                super::time::to_us(task.created_at),
                super::time::to_us(task.updated_at),
            ],
        )
        .await
        .map_err(|e| StorageError::Internal(anyhow::anyhow!("libsql insert session_tasks: {e}")))?;
        Ok(())
    }
    ...
    async fn get(&self, session_id: &SessionId, task_id: &TaskId) -> Result<Option<Task>> {
        ...
        let mut rows = conn
            .query(
                &sql,
                libsql::params![session_id.as_str().to_string(), task_id.to_string()],
            )
            .await
            .map_err(|e| {
                StorageError::Internal(anyhow::anyhow!("libsql query session_tasks: {e}"))
            })?;
        let row = rows.next().await.map_err(|e| {
            StorageError::Internal(anyhow::anyhow!("libsql session_tasks row: {e}"))
        })?;
        Ok(row.and_then(|r| try_task_from_row(&r)))
    }
    ...
    async fn list(&self, session_id: &SessionId) -> Result<Vec<Task>> {
        ...
        let mut rows = conn
            ...
            .await
            .map_err(|e| {
                StorageError::Internal(anyhow::anyhow!("libsql list session_tasks: {e}"))
            })?;
        let mut out = Vec::new();
        while let Some(row) = rows
            .next()
            .await
            .map_err(|e| StorageError::Internal(anyhow::anyhow!("libsql session_tasks row: {e}")))?
        {
            if let Some(task) = try_task_from_row(&row) {
                out.push(task);
            }
        }
        Ok(out)
    ...
    async fn update(&self, session_id: &SessionId, patch: &TaskPatch) -> Result<bool> {
        ...
        if let Some(depends_on) = &patch.depends_on {
            params.push(libsql::Value::Text(depends_on_json(depends_on)?));
        ...
        params.push(libsql::Value::Text(patch.task_id.to_string()));
        ...
        let affected = conn.execute(&sql, params).await.map_err(|e| {
            StorageError::Internal(anyhow::anyhow!("libsql update session_tasks: {e}"))
        })?;
        Ok(affected > 0)
    }
    ...
    async fn delete(&self, session_id: &SessionId, task_id: &TaskId) -> Result<bool> {
        ...
        let affected = conn
            ...
            .await
            .map_err(|e| {
                StorageError::Internal(anyhow::anyhow!("libsql delete session_tasks: {e}"))
            })?;
        Ok(affected > 0)
    }
}
...
mod tests {
    ...
    async fn store_with_session(id: &str) -> (LibsqlTaskStore, SessionId) {
        let pool = LibsqlPool::open_in_memory().await.unwrap();
        let sessions = LibsqlSessionStore::new(pool.clone());
        sessions.save(&make_session(id)).await.unwrap();
        (LibsqlTaskStore::new(pool), SessionId::from(id))
    }
    ...
    async fn tasks_cascade_delete_with_session() {
        ...
        let store = LibsqlTaskStore::new(pool);
