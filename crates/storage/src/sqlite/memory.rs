...
use aura_memory::{MemoryError, Result};
...
impl MemoryStore for SqliteMemoryStore {
    async fn store(&self, entry: &MemoryEntry) -> Result<()> {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            .map_err(|e| MemoryError::Internal(anyhow::anyhow!("sqlite insert memory: {e}")))?;
            Ok(())
        })
        .await
        .map_err(|e| MemoryError::Internal(anyhow::anyhow!("spawn_blocking join: {e}")))?
    }
    ...
    async fn retrieve(&self, user_id: &str, key: &str) -> Result<Option<MemoryEntry>> {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let mut stmt = conn
                ...
                .map_err(|e| MemoryError::Internal(anyhow::anyhow!("sqlite prepare: {e}")))?;
            let result = stmt
                ...
                .optional()
                .map_err(|e| MemoryError::Internal(anyhow::anyhow!("sqlite query: {e}")))?;
            match result {
                Some(data) => {
                    let entry: MemoryEntry = serde_json::from_str(&data).map_err(|e| {
                        MemoryError::Storage(format!("deserialize memory entry: {e}"))
                    })?;
                    Ok(Some(entry))
                }
                None => Ok(None),
    ...
    async fn search(&self, user_id: &str, query: &str, limit: usize) -> Result<Vec<MemoryEntry>> {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            for row in rows {
                let data =
                    row.map_err(|e| MemoryError::Internal(anyhow::anyhow!("sqlite row: {e}")))?;
                let entry: MemoryEntry = serde_json::from_str(&data).map_err(|e| {
                    MemoryError::Storage(format!("deserialize memory entry: {e}"))
                })?;
                entries.push(entry);
    ...
    async fn delete(&self, id: &str) -> Result<()> {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            conn.execute("DELETE FROM memories WHERE id = ?1", rusqlite::params![id])
                .map_err(|e| MemoryError::Internal(anyhow::anyhow!("sqlite delete memory: {e}")))?;
            Ok(())
        })
        .await
        .map_err(|e| MemoryError::Internal(anyhow::anyhow!("spawn_blocking join: {e}")))?
