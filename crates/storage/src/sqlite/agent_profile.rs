...
impl AgentProfileStore for SqliteAgentProfileStore {
    ...
    async fn create(&self, row: &AgentProfileRow) -> Result<()> {
        ...
        let reasoning_effort = row.reasoning_effort.map(|e| e.as_str().to_owned());
    ...
    async fn update(&self, id: &AgentProfileId, update: &AgentProfileUpdate) -> Result<bool> {
        ...
        let reasoning_effort = update.reasoning_effort.map(|e| e.as_str().to_owned());
        let now = super::time::now_us();
        let outcome = self
            .pool
...
mod tests {
    ...
    /// Raw column read that bypasses `row_from_raw`: proves the empty set is
    /// stored as SQL `NULL`, not the string `"[]"`.
    async fn raw_allowed_models(
        ...
        store
            .pool
            .interact("test.raw_allowed_models", move |conn| {
                Ok(conn.query_row(
    ...
    async fn corrupt_column(
        ...
        store
            .pool
            .interact("test.corrupt_column", move |conn| {
    ...
    async fn allowed_models_and_effort_round_trip() {
        ...
        row.reasoning_effort = Some(ReasoningEffort::High);
        store.create(&row).await.unwrap();
    ...
    async fn corrupt_effort_or_set_column_errors_on_read() {
        ...
        corrupt_column(
            &store,
            &row.id,
            "UPDATE agent_profiles SET reasoning_effort = NULL, allowed_models = 'not-json' \
             WHERE id = ?1",
        )
        .await;
        assert!(
