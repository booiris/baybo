...
Container lifecycle: one container per Aura process, named `aura-browser-<pid>-<rand6>`, labelled `aura.role=browser-sidecar` + `aura.pid=<pid>`. `docker run` is **not** invoked with `--rm` (deliberate — `--rm` deletes the container before we can fetch logs on a startup crash, leaving a useless `(no logs)` diagnostic). Cleanup is owned by the success-path `stop()` (`docker stop -t 5 <name>` then `docker rm -f`), the post-`docker-run` failure path (force-remove during error recovery so the host-headless fallback doesn't fight the container for the profile), and the next-boot `sweepStaleContainers()` pass (handles `kill -9` Aura crashes by removing labelled containers whose `aura.pid` is dead). Inside the container Chrome listens on loopback only (Chrome 134+ silently ignores `--remote-debugging-address=0.0.0.0` for DNS-rebinding protection); a `socat 0.0.0.0:9223 → 127.0.0.1:9222` relay in the entrypoint forwards the published port. The container therefore publishes its CDP on `127.0.0.1::9223` (ephemeral host port chosen by docker, read back via `docker port`); multiple Aura instances on one host don't collide.
