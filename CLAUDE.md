...
Every in-tree channel sidecar under `channel-src/*` ships inside the aura binary as a zstd-compressed JS bundle. `crates/gateway/build.rs` runs `pnpm --filter <pkg> bundle` (which invokes esbuild to emit a single self-contained ESM at `dist/bundle.mjs`), zstd-compresses each bundle plus any aux assets, and emits `$OUT_DIR/sidecar_assets.rs`. At boot, `SidecarRuntime::install` materialises everything to `$XDG_CACHE_HOME/aura/sidecars/<channel>-<hash>/{bundle.mjs, <aux...>}` (hash-keyed so upgrades never overwrite the old files — a downgraded install can still find its bundle). `SidecarSupervisor` then runs one restart loop per embedded channel type and spawns each via the host's `node` (resolved from `PATH`, override with `AURA_NODE_BIN`).

- Pre-reqs before `cargo build`: `pnpm install` must have populated `channel-src/*/node_modules` and `sdks/channel-ts/dist`, and `node` must be on `PATH` to run sidecars at runtime (not needed at build time — `pnpm bundle` invokes esbuild directly). Missing the install degrades to `cargo:warning=…` and empty embedded assets — the build still succeeds, the supervisor logs "embedded sidecar runtime unavailable".
- Adding a new sidecar: create `channel-src/<name>/` with `src/index.ts` calling `runSidecar`, add it to `pnpm-workspace.yaml`, copy the `"bundle": "esbuild …"` script and `esbuild` devDependency from one of the existing sidecars, and run `pnpm install`. `build.rs` picks it up automatically via the `channel-src/*` enumeration — no Rust changes needed.
- **Do not add `--minify` to the `bundle` script.** esbuild's minifier makes `grammy` + `node-fetch@2`'s outbound HTTPS requests deadlock (e.g. `bot.init()` never returns; long-poll never starts). Reproduces with a 5-line `import { Bot } from "grammy"; await bot.init();` smoke test built with vs. without `--minify`. Bundles are ~30% larger without it but actually work; suppress that itch.
