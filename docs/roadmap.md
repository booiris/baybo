...
- **Desktop app** — *on a branch*. A Tauri 2 thin client wrapping the web UI
  ...
  heavily bitrotted.
- **Android support** — *in progress*. An Android counterpart to the iOS companion app:
  the shared Rust core and transcript bundle now live in `app/mobile/`, linked
  by both shells; still to come are the Android platform seams, the
  Kotlin/Compose shell at `app/android/`, and a production FCM sender in
  remote-host. Spec:
