...
**User-facing behavior.** Each row is one cron fire (`ChatCronMessage`, keyed by `session_id`). Collapsed, it shows the fire's **instruction** (the first non-empty line of the `prompt`, else `Cron fire`) as the title, a relative fire time (`formatRelative`: `HH:MM` today, `Mon DD` within a week, else `YYYY-MM-DD`; absolute `fired_at` is in the row `title` tooltip), and — as the preview — the assistant `response` collapsed to one line, or a spinner + "Awaiting response…" while the agent hasn't replied. Lengths are left to CSS (`truncate` / `line-clamp-2`) so the full text stays available on hover. Clicking a row expands it to show the full `Prompt`, `Response` (rendered `(pending)` when the agent hasn't replied yet), and an id footer with the `cron_job_id` (truncated to 8 chars) + the full `session_id`; clicking again collapses it. The header shows a count of unread fires and a **mark-all-read** button; an unread fire carries a left brand accent bar, a bold title, and a bordered surface card. A refresh button forces an immediate refetch.
...
**Unread state is client-only.** There is **no server read-state** for cron fires. Acknowledged fires are tracked entirely in `localStorage` under `SEEN_KEY = 'baybo.cron.seen'`, a JSON array of acknowledged `session_id`s, mirrored into React state (`seen: Set<string> | null`). A fire is acknowledged when its row is opened or collapsed (`toggle` → `markSeen`) or via mark-all-read (`markAllSeen`). `unreadCount` is the count of list items whose `session_id` is not in `seen`.
