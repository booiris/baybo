...
### List ordering: most-recent user message first, with pinned lifted out

Incoming server order is newest-first, and the one live update that reorders the list is a **user-authored message**: the conversation the user just spoke in moves to the front, whether they typed it in this tab, in another tab, or on the phone. Everything else merges in place. `applySessionPatch` and `applySessionActivity` both merge fields onto the existing row by index and explicitly leave position untouched — a title patch, a pin, or an assistant reply must not reshuffle the list under the user. The move itself is one three-line helper, `bumpSessionToFront`, called from exactly two places: `applySessionUserMessage` (this tab's sends and the inbound user echoes) and the `source === 'user'` arm of the `session_activity` handler — the latter being the *only* signal for a session this tab isn't subscribed to, since the user echo reaches subscribers alone. A genuinely new session (a patch for an unknown `session_id` carrying `created_at` + `last_active`) is **prepended**. `/stop` counts as a user message here, as it already does for the preview text — it is typed by the user, echoed as a user bubble, and reaches every client's pulse — with one consequence worth knowing: the router answers a stop *before* it touches `last_active` (`crates/agent/src/actor/router/user_input.rs`), so a stop is the one raise the server does not record, and the row settles back into `last_active` order at the next refetch.

The bump agrees with the server instead of fighting it, which is why it needs no client-side clock: `GET /v1/chat/sessions` sorts `last_active DESC` (`crates/storage/src/sqlite/session.rs`) and the gateway touches `last_active` on user input (`crates/agent/src/actor/router/user_input.rs`), so the next refetch reproduces the same order. `bumpSessionToFront` therefore moves position only and writes no field — no `last_active` on a live row is ever client-invented: `applySessionActivity` projects the server-stamped `at` off the pulse that comes back for the same message, and `applySessionPatch` merges whatever a full Created/unhide patch carries — both of them the server's own value. (The one locally stamped timestamp is the one a chat created in this tab is born with, until that Created patch lands on it.) Within `SessionSidebar`, the rows are then bucketed (`useMemo`, ~511): every `pinned` session is lifted entirely out of the folder tree into the top `Pinned` block; non-pinned sessions are grouped by `folder_id`; a session whose `folder_id` points at a folder that isn't *reachable* in the rendered tree (dangling parent, cycle, or depth ≥ 2 grandchild) falls back to `Uncategorized` rather than vanishing. Bucketing preserves the incoming order within each block, so "front of the list" means front of the row's own block: a pinned chat tops the `Pinned` block, a foldered chat tops its folder's chats, and a cron fire tops its group. A cron *group* has no array position at all — it sorts by its newest member's `last_active` — so a user reply into a fire floats the whole group only once the server's activity pulse lands. The `Pinned` block only renders when at least one session is pinned.
...
The row title renders `session.title` — a short summary of the user's **opening message** (its first question, plus the images sent with it when the lite model would see them as pictures, so "what's this?" under a screenshot still names what the screenshot shows), generated server-side unless the user has renamed the conversation — falling back to `session.last_user_text` (the session's most-recent user-authored message, truncated) and then the italic placeholder `New conversation` when neither exists yet. The **title** is generated once, server-side, by the agent's title pass when a session's first user turn commits — and only while the conversation still has none, so a rename is never overwritten by a title pass that was already in flight (see [`docs/modules/agent.md`](modules/agent.md) → *Conversation title*); it arrives via a `Frame::SessionUpdated` patch carrying `title` (merged by `applySessionPatch`, which also drives the chat header's `activeTitle`) and is seeded on history load from `ChatSessionSummary.title` / `ChatSessionDetail.title`. The `last_user_text` **preview** (the fallback) is updated locally without a list refetch on this tab's own send and on inbound user echoes via `applySessionUserMessage`, which collapses whitespace and truncates to `PREVIEW_MAX_CHARS` (120) to mirror the server's `truncate_preview` — and, in the same call, moves the row to the front. Preview and position are folded into one helper on purpose: they are two halves of "a user message landed here", and a caller that picked up one and forgot the other would be the obvious bug. The move is deliberately not gated on the preview having changed, or re-sending the same string would leave the row where it was.
...
- `Frame::SessionActivity { session_id, source, at }` → `applySessionActivity` (drives `last_active` freshness + the unread bump, as above), then `bumpSessionToFront` when `source === 'user'`.
- User-echo paths → `applySessionUserMessage` (drives the preview title, and moves the row to the front).

Invariants worth calling out, since they're easy to misread: (1) the only live update that repositions a row is a **user-authored message** (creation prepends; everything else merges in place), so the order is stable under concurrent replies but does follow the user's own traffic; (2) the unread counter is **server state** (`unread_count`, off the session's server-side read cursor) that live activity pings only accelerate, and is always 0 on the foregrounded row; (3) the timestamp/age is recomputed from `last_active` on each render, not persisted; (4) the trash icon is hide-semantics, never a real delete; (5) `sessions` is the server's list verbatim — archived rows live in it and are dropped only where the list is drawn (see above), so nothing that reads the raw array may assume every entry has a row on screen.
...
Crucially, **chats are plain `useDraggable`, not sortable** — they are deliberately kept out of `SortableContext` (`allSortableIds` contains only folder ids). Chat list order is recency-driven (most-recent user message first within each bucket) and is **not** hand-arrangeable; dragging a chat only lifts it out to re-file it, and never reflows the list. Folders use `useSortable` (which registers the same id as both the sortable and the drop target via one `setNodeRef`/`isOver`, avoiding a double-registration race).
...
- **Chat → another chat** → ignored (order is recency-driven, never hand-arranged).
...
Fenced code blocks render through `components/MarkdownCodeBlock.tsx`, shared by chat replies, work-block prose, project run transcripts, and issue timeline comments. A language tag such as `typescript`, `rust`, or `swift` selects the corresponding `highlight.js` grammar; an untagged fence is auto-detected from the registered language set, while `text` / `plaintext` fences stay deliberately uncoloured. The highlighter escapes the source before emitting token spans, and an unknown language or a highlighter failure falls back to the original code rather than taking the message through `MarkdownFallback`. Token colours and the code surface follow Atom One Light; the Baybo border and copy-button chrome remain local UI styles.

Every block keeps a visible copy control in its upper-right corner. It copies the raw source without the fence or CommonMark's trailing newline, confirms with a check for 1.2 seconds, and falls back to a temporary selected textarea when the Clipboard API is unavailable (for example an insecure LAN origin).

The issue description is an always-live Milkdown editor rather than `MarkdownBody`, so it reaches the same feature through `projects/markdownCodePlugin.ts`: a code-block node view owns the copy control, and ProseMirror inline decorations carry the same `hljs-*` token classes without replacing the editable text DOM. `chatMarkdown.test.tsx` pins rendered blocks and copying; `issueRefsProse.test.tsx` pins the editor path.
...
- **`session_activity { source, at }`** — cheap unread/freshness signal delivered to **every** connection regardless of subscription. Foreground session doesn't bump; background gets a `+1` badge. `source` is load-bearing for ordering: `user` moves the row to the front of the list, `assistant` leaves it where it is. An activity for an unknown session id triggers a session-list refetch, so a conversation this tab has never seen (e.g. a recurring cron fire opening its own) appears in the sidebar.
...
- **The outbox** (`pages/chat/outboxStore.ts`, localStorage `baybo.outbox.<sessionId>` — same pattern as the interjection queue's store): entries are keyed by `platform_msg_id` with a `sending → sent → (released)` / `failed` / `unknown` state machine. Confirmation is **two-stage**: the ordinal-less Echo proves *transport* (`sending → sent`, in-connection retries stop); an **ordinal-stamped row with the same key** — from sync or backfill — proves *durability* and releases the entry. No Echo within 10 s → one blind in-connection resend (safe inside the gateway's dedup window); hard cap 3 automatic transmissions, then `failed` + the red manual-retry affordance (which reuses the same key). On every reconnect edge the affected sessions (subscribed ∪ outbox) recover in parallel, and within each, sync runs first — the reconciliation gate — before entries still lacking durability confirmation resend. Every `Message` / `Messages` send crosses one `ChatWs` subscription barrier: if its session is not on the current registered socket, `Subscribe` is queued first on that same socket, and the WebSocket ordering guarantee makes the gateway observe it first. This includes outbox-only sessions recovered before `/chat` has selected its active URL session. A failed resend enqueue consumes no additional retry transmission. After a **rebased** sync the page can't prove absence (the floor is hidden), so unconfirmed entries go `unknown` and resolve via the per-key point lookup (`GET /v1/chat/sessions/:id/messages?platform_msg_id=…`): found → released, provably absent → the retry machine resumes. After any REPLACE (baseline/rebase), locally-rendered rows whose key is still in the outbox and absent from the page are re-appended (the REPLACE-overlay rule) so an echoed-but-unpersisted send never vanishes from screen.
