...
|---------|--------|-----------|---------|
| `git` | **Required at startup** | `baybo-workspace` (`manager.rs`) | `git init` of the workspace identity repos (`config/`, `profile/`, `skills/`, `agents/`). Baybo surfaces a startup error if it's genuinely missing. Also used best-effort by the Edit tool to auto-commit `profile/` identity-file edits (failure degrades to a `commit_warning` in the tool output), and by `baybo-deck` (`repo.rs`) to auto-commit each card-bundle mutation (install/update/purge) into a `workspace/deck/` repo for version history (failure degrades to a `deck::provenance` warn; the deck operation still succeeds). |
