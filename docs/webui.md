...
- TS tooling is **pnpm** repo-wide (the root workspace's lockfile is `pnpm-lock.yaml`, with members declared in `pnpm-workspace.yaml`: `app/web`, `sidecars/*/*`, `bench/bench-web/web`; the iOS transcript webview `app/mobile/web` is a standalone pnpm project with its own lockfile — not a stray). Never invoke `npm` — `npm ci`, `npm install`, `npm run …` all route through `pnpm` equivalents below.
...
- Wire the escape hatch into your editor's rust-analyzer config (`rust-analyzer.cargo.extraEnv`) so an in-progress `app/web` edit doesn't redden the Rust tree — editor config dirs (`.vscode/`, `.zed/`, `.idea/`) are gitignored, so each user sets it locally. Don't export it shell-wide: a terminal `cargo build` is the only thing that tells you the dashboard is broken.
