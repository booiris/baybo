...
| `pages/cronActions.test.ts` (29) | `CronPage.tsx` — `actOnCronJob`, `fetchCronJobs`, `fetchGrantableMcpTools`, `updateCronJob`, `deleteCronJob`, `cronEditPatch`, MCP grant grouping/replacement/stale helpers, … | Cron page API helpers driven against a `FakeGateway` (real `openapi-fetch` client): soft delete, pause/resume, expired-one-shot resume 400, partial-patch reschedule, exact MCP grant listing/replacement/revocation and stale-no-refresh behavior, error slots. |
