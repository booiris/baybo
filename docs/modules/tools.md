...
| `WebFetch`                                                                                                                                                                                                                                                            | implemented | renders the response as Markdown; when `prompt` is supplied AND `default_tools` was given a side `LlmCompletion` (gateway/runtime path passes `Some`, argv-mode passes `None`), runs a fixed-system extraction pass and returns the model's reply instead of the raw body. Without an LLM the prompt is silently ignored. |
| `SendFile`                                                                                                                                                                                                                                                            | implemented | streams a local file into `BlobStore` and returns a channel attachment                                      |
| `UpdateProfile`                                                                                                                                                                                                                                                       | implemented | writes `profile/{SOUL,USER,IDENTITY}.md` (selected by `target` enum, never an arbitrary path); supports `mode=replace` (unique-or-fail unless `replace_all`) and `mode=append` (auto trailing `\n`); commits the single touched file to the `profile/` git repo with a fixed `Aura <aura@local>` author and `--no-verify` (audit history, not a hand-curated repo); detached HEAD or commit failure leaves the file write in place and surfaces a Warn `Notice` instead of an error. **Bypasses approval** — visibility is via the post-success `NoticeLevel::Info` `Notice` plus the in-`profile/` git history. Changes do **not** affect the current session's `Soul` (system prompt is baked at `AgentLoop` construction); see [`docs/todo/profile-hot-reload.md`](../todo/profile-hot-reload.md). |
