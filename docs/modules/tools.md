...
- `ExecCommand` — exact full-command string match (no shell tokenization). `BashTool` declares this `ResourceAccess` for every executable command when `permission=manual` and `sandbox_permissions=use_default`; FileToolRedirect commands (`cat foo`, `sed -i …`) are rejected before any approval prompt. `auto` owns the destructive-command gate internally through the risk judge, and `free` disables Bash pre-execution approval by running directly outside the OS sandbox. Under `auto` or `manual`, `sandbox_permissions=require_escalated` declares nothing to the executor and instead owns a fresh uncached prompt inside Bash, so a cached approval for the sandboxed command cannot authorize the wider route. Under `free`, it is a no-op because the route is already unsandboxed.
