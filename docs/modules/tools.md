...
| `Bash`                                                                                                                                                                                                                                                                | implemented | `sh -c` under the configured `permission` policy. `auto` and `manual` use the OS sandbox in **permissive filesystem** mode when an inner sandbox runner is available; if Baybo detects an outer container/sandbox, Bash silently skips the inner sandbox, and if no backend is available on a non-container host, Bash warns and runs without it. `free` runs directly while keeping the tool-layer work-dir jail and uv shim. A call may set `sandbox_permissions=require_escalated` only when the user explicitly requests unsandboxed execution; Baybo shows the exact command in a fresh approval prompt before changing a sandboxed route. Under `permission=free`, the route is already unsandboxed, so the override needs no prompt. Network enabled. No env/cwd persistence across calls. See `docs/modules/sandbox.md#filesystem-policy-workspace-vs-permissive`. |
...
| `WebSearch`                                                                                                                                                                                                                                                           | implemented | dynamic tool owned by `baybo-search`; returns ranked titles, URLs, and snippets through Tavily, Brave, or SearXNG. It never fetches result pages and is registered only when the configured provider can run. See [`web-search.md`](web-search.md). |
...
| `DeckCardList`, `DeckCardGet`, `DeckCardCreate`, `DeckCardUpdate`                                                                                                                                                                                                                                     | implemented | live in `baybo-deck::tools` (hold `Arc<DeckManager>`); registered from `crates/baybo/src/runtime.rs` after the deck manager is constructed. **Authoring pair** (`Create`/`Update`): install / replace a card from a staged bundle directory; both run the dry-run gate (static validation with mandatory response schemas → real host boot → one refresh-op invocation → successful schema-checked first snapshot) before anything goes live. Null, oversize, off-schema, and top-level `error` first results fail the tool call; success returns a bounded structural `first_snapshot` summary without copying its value into the agent transcript. Runtime op results and `ctx.emit` snapshots use the same response validators. `Trusted` + `ReadFile` (they read the agent's staged dir). **Discovery pair** (`List`/`Get`): `List` returns every live card's id/title/size/enabled/spec_hash; `Get(card_id)` returns the card's four source files inline — together they let the agent update a card it didn't author in this conversation (resolve description → uuid, edit from real source) since its file tools can't reach the deck root; `Trusted`, no filesystem capability (they read through the manager). All four are `channels: [owner]`. See [`deck.md`](deck.md) |
...
`LivePermissionMode` handle described in the permission policy section — shared with `Write` and `Edit`, which read it for the `free` waiver.
...
The manifest also carries `channels: Vec<ChannelType>` (empty = every channel, the norm). It is one of **three** axes that decide which tools a session is offered; the second is `Tool::trigger_scope() -> ToolTriggerScope` (`Any` / `CronConversation` / `ProjectBoard` / `SharedWorkspace`), and the third is the subagent profile's own `tools:` list (below). The first two are enforced twice — the agent loop assembles the LLM's tool list via `ToolRegistry::tool_definitions_for_session(&session.channel, &session.trigger)` so other sessions never see the tool (channel and trigger are both session-stable, so the list stays byte-identical across calls and the prompt-cache sort guarantee holds), and the executor refuses a call that names it anyway (omission is not a gate against a hallucinated or skill-body-prompted name).

Each `trigger_scope` variant restricts to a range: it names where the tool would have **nothing to act on** — `report_nothing` has no fire to silence outside a recurring cron conversation, `Issue*` has no board to name outside an issue run or project-linked cron fire, the browser server is one process-wide Chrome and a card's run was cut its own checkout precisely so it would not share — so offering it elsewhere would be offering an action that can only fail. An MCP server carries the axis too, as `McpServerEntry::trigger_scope` in `.mcp.json` (`#[serde(default)]` = `Any`, so a file written before the field kept every server everywhere); every tool the reconciler registers for that server answers with it, and the value is folded into the server's identity hash so an edit reconnects rather than leaving the previously-registered tools in place. The deck tools (`DeckCardList`/`DeckCardGet`/`DeckCardCreate`/`DeckCardUpdate`) carry `channels: [owner]` — the deck is the owner's surface — *and* `SharedWorkspace`, because an issue run is dispatched on the owner channel too and the channel axis alone never kept them out of one.

The third axis is a **budget, not a boundary**, and is the only one of the three that isn't: a subagent profile's frontmatter may carry `tools: Read, Grep, …` (comma-separated — the profile frontmatter parser is a scalar-only YAML subset), and a child on that profile is offered only those, intersected with what its channel and trigger already allow. No key means unrestricted, so a profile written before the key — or loaded from outside the workspace — keeps what it has. The executor does **not** refuse a name outside the list; it is there to keep a read-only child from carrying a 60 KB tool block it will never call. Three of the four built-ins name a list (`explorer`, `planner`, `reviewer`); `general-purpose` exists to mutate the workspace and names none. The list is pinned onto the child's session row at spawn, beside `subagent_type` — the spawn tool holds the profile, the spawner holds the row — so it is fixed for the session's life like the other two axes and cannot move when the profile file does.
...
- `ExecCommand` — exact full-command string match (no shell tokenization). `BashTool` declares this `ResourceAccess` for every executable command when `permission=manual` and `sandbox_permissions=use_default`, including commands that read or edit files such as `cat foo` and `sed -i …`. `auto` owns the destructive-command gate internally through the risk judge, and `free` disables Bash pre-execution approval by running directly outside the OS sandbox. Under `auto` or `manual`, `sandbox_permissions=require_escalated` declares nothing to the executor and instead owns a fresh uncached prompt inside Bash, so a cached approval for the sandboxed command cannot authorize the wider route. Under `free`, it is a no-op because the route is already unsandboxed.
...
Each call also carries `sandbox_permissions`: `use_default` (the default) keeps
the configured route, while `require_escalated` is reserved for an explicit
user request to run that command without the OS sandbox. The latter is a
per-call privilege change, not a standing mode: Bash raises a fresh uncached
approval containing the exact command, then selects the unsandboxed route only
after approval. An optional `justification` supplies the user-facing question
or reason at the top of that prompt; empty or omitted input uses Bash's generic
question, while the exact command and fixed host-access warning are always
shown. A cached approval for the same command's sandboxed form does not cover
it, and an unattended context with no approval surface fails closed. Under
`permission=free`, no privilege transition occurs: the command is already
unsandboxed, so `require_escalated` and its `justification` are no-ops. The
tool-layer work-dir jail and uv shim remain in force. In the disposable
`bench-bash` profile, which is also already unsandboxed and deliberately has no
approval surface, both fields are no-ops.
...
All other tools (`Read`, `Write`, `Edit`, `Echo`, `Now`, `WebSearch`, `Secret*`, `JobList`, `JobStop`, `Cron*`, `DeckCard*`, `SkillUninstall`, `Task*`, the other `viking_*` tools, every `todo` stub) use the default — they're either pure data ops or fail fast.
...
- **`ToolConcurrency::Concurrent`** — safe to run alongside other concurrent calls; declared by the read-only builtins: `Read`, `Glob`, `Grep`, `WebFetch`, `WebSearch`, `Now`, `Skill`, `CronList`, `TaskGet`, `TaskList`, `SecretList`, `SecretCheck`. They read (filesystem, network, or a store) and mutate no shared state, so parallel calls within a turn cannot race. `AttachFile` and `PutBlob` stay exclusive — staging a blob to send or reference is an outward-facing write, kept on the clean "concurrent = pure read" side of the line. `GetBlob` reads only (a `stat` and a path), so it joins the concurrent set.
- **`ToolConcurrency::Exclusive`** (the default) — runs alone among pool calls. Every mutating builtin (`Write`, `Edit`, `MemoryDelete`, `Bash`, `SecretAdd`, `Cron{Create,Update,Delete,Pause,Resume}`, `DeckCard{Create,Update}`, `Skill{Install,Uninstall}`, `Task{Create,Update}`), dynamic tool that does not explicitly opt into another mode, and tool the registry cannot classify falls here. A tool with side effects must never overlap a reader (read-while-write race) or another writer.
...
Tool output should prefer structured `Json`, use `LargeText` for long text with truncation, and be sanitized before entering Turn or Trace. `WebFetch` and `WebSearch` return `Text` because their output is prose the model reads directly.
