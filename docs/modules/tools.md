...
| Tool                                                                                                                                                                                                                                                                  | Status      | Notes                                                                                                       |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- | ----------------------------------------------------------------------------------------------------------- |
| `Read`, `Write`, `Edit`                                                                                                                                                                                                                                               | implemented | file I/O on absolute paths. `Edit` carries extra guards when `file_path` resolves under `<workspace>/personas/`: writes are restricted to the three identity files (`SOUL.md`, `USER.md`, `IDENTITY.md`) inside the *calling agent's own* persona directory, the existing file is capped at 1 MiB, and after a successful write the change is staged and committed to `personas/`'s standalone git repo with a fixed `Baybo <baybo@local>` author and `--no-verify` (audit history, not a hand-curated repo). Detached HEAD or commit failure leaves the file write in place and surfaces a `commit_warning` in the tool output. A profile edit does **not** change the running session's system prompt mid-turn; `ContextManager` re-resolves it on the next compaction (see [`agent.md`](agent.md)), so the edit takes effect from then on. |
