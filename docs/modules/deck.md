...
- **Optional `src/` + host `bun build`.** `card.html` stays one self-contained built file, but a complex card may author it from TypeScript/modules under `src/` and build with `bun build` on the host. `src/` rides along with the bundle (install copies it under caps: ≤32 files / ≤256 KB each / ≤2 MB total, no symlinks; the gateway never runs or reads it) purely so **`DeckCardGet` returns it** for a cross-chat edit from the real inputs. `install`'s `materialize` went from copying four fixed files to the four files plus the `src/` subtree.
