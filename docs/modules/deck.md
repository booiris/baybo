...
- **The deck shell ships in the transcript dist**, as a second Vite entry (`deck.html`) served at `baybo-transcript://localhost/deck.html` — not a separate `App/Resources/deck` copy step. One dist, two entries; `build-app.sh` and `project.yml` are unchanged, and no CI path-filter change is needed (the shell lives under the already-filtered `app/mobile/web`; `crates/deck` is not an ffi dependency).
- **Provenance events are structured tracing** (target `baybo_deck::provenance`: install / update with hash before→after / delete / restore / purge / quarantine / sdk-regate), not `TraceStore` spans — deck lifecycle has no session to hang a trace tree on; store-backed spans are deferred with that design question.
- **The install-time rendered preview screenshot** (browser-sidecar best-effort in the original design) is not implemented. The implemented authoring check is deterministic instead: create/update returns a bounded structural summary of the schema-checked first snapshot for the agent to inspect; the snapshot value remains on the Deck data plane.
- **iOS surfaces**: the FFI exposes fetch/recycle/bundle/call/layout/enable/disable/delete/restore/purge but not the per-card `openapi.json` (REST-only). The recycle bin has a native screen (`DeckRecycleScreen`, pushed from the Deck header's ☰ menu — the Chats-☰ grammar); rows restore directly, while a long-press menu or no-full-swipe destructive action requires a native alert before `deck_purge` permanently removes the row, snapshots, bundle, runtime residue, and `deck:<card_id>` blobs. Both mutations are optimistic in the bin and roll the row back on failure; the board refetches on the `DeckChanged` echo. The native confirms are system alerts rather than the hand-rolled `ConfirmDialog`. The empty board is also native: `DeckScreen` covers the warm shell with the shared `CreationPrompt`, using the real `rectangle.stack` SF Symbol and calling `AppStore.startCardDraft` directly; the web shell starts at the card grid, not a second empty-state implementation.
...
The process runs **directly on the gateway host — no OS sandbox** — spawned like the channel sidecars, inheriting the host environment so a card can reach host state, installed CLIs, and credential dirs. `bun` itself is located by `baybo_process::HostTool::bun()` rather than by the inherited `PATH` alone: under a service manager that `PATH` is the manager's, not the operator's, so a `~/.local/bin/bun` the operator uses daily is otherwise invisible (see "The daemon's `PATH` is not the operator's" in [`docs/external-commands.md`](../external-commands.md)). Note the second-order case the resolver does *not* cover — a card whose script shells out to `codex` or `claude` by name via `ctx.exec` needs those on the daemon's real `PATH`, which is what the installer's generated `PATH=` line is for. This is a deliberate reversal of the original design (which ran the service under `crates/sandbox` with `NetworkPolicy::None`): sandboxing bun broke binary resolution (bwrap's scrubbed `PATH` never found `bun`) and, more fundamentally, walled the service off from exactly what real cards need — a Codex-quota card must read `~/.codex` credentials and reach the network; a machine-status card must run host CLIs. The sandbox was never a security boundary against the author anyway (see the trust model below), so it was removed rather than perforated. The remaining bounds are all app-level:
...
**A host fault is not a card fault, and never quarantines.** If the JS runtime cannot be launched at all (`DeckError::HostToolMissing` — no `bun` on the daemon's `PATH`), the card's code never ran, so nothing about the failure is attributable to the card and the *same* failure hits every card on the host. The supervisor backs off and retries indefinitely, spending no strikes: quarantining would fix nothing and would write durable state (`enabled = 0` + `quarantined_at`) that the operator must undo card-by-card after repairing the host. This is not hypothetical — putting the gateway under systemd changed its `PATH`, and within eight seconds every enabled card was disabled and stamped, each labelled with a reason (`crash budget exhausted`) that described a fault none of them had.

The same reasoning covers a bundle the gateway can no longer **load**: a contract the running code tightened, or a file damaged under it, is not the card misbehaving, and `boot` cannot author the fix. `DeckManager::mark_unloadable` records the reason as the card's error face and leaves the row enabled and unstamped, so the next boot retries and `enable` can re-admit it once the bundle is repaired. Quarantine would be a one-way latch here, because `boot` skips an already-quarantined row on every later boot — the card would never retry even after the bundle was fixed.

**The stored reason names the cause, not the trigger.** `quarantine_reason` carries the last failure into the string the sink persists (`crash budget exhausted: <cause>`), because that snapshot is what the card's error face shows and it is the only copy that outlives the log.
...
Each service publishes its op surface as its own `openapi.json`, and the spec is **load-bearing, not documentation**: at install the gateway parses it into compiled per-op request and result validators. Every call that **crosses the gateway** — user-initiated ops and every dry-run invocation (install, update, re-admission) — validates the op and params *before* agent-written code runs, then validates the returned JSON before forwarding it. Runtime `ctx.emit` payloads validate against the refresh op's same result contract before storage/broadcast. The document wrapper stays a hand-checked narrow subset: op-name grammar, `get`/`post` only, mandatory `x-baybo-retryable`, and exactly a mandatory `responses.200.content.application/json.schema` plus optional `responses.default...schema` per op. The requirement is an **admission** rule, not a load rule: `DeckCardCreate`/`DeckCardUpdate` refuse a bundle whose ops declare no `200` schema, while `load_bundle` accepts one and leaves its results unchecked (`spec::ResponseSpec::Undeclared`). Cards installed before the contract existed therefore keep running and keep their place on the deck; they gain checking the next time their author updates them. Enforcing at load instead would have made an upgrade disable every pre-contract card at once — the failure mode the host-fault rule below exists to prevent — with no path to author the missing schema from inside `boot`. Both request and result checks are **standard JSON Schema compiled by the `jsonschema` crate**. Parameter schemas are embedded in a per-op `{type: object, properties, required, additionalProperties: false}` wrapper; result schemas are compiled verbatim. `$ref`/`$dynamicRef` are rejected throughout, and the crate is built `default-features = false`, so an agent-written schema can never make compilation touch the network or filesystem.

The top-level field `error` is a reserved result discriminator. A result containing it must match the optional `default` schema; all other results must match `200`. Runtime ops and emits may carry a declared graceful error so the card can render a temporary upstream outage, but the dry-run gate rejects such a result even when it is schema-valid: a new install/update must prove one successful refresh. Off-schema results are never stored or forwarded. The self-timer's internal op invocation remains an in-process JS call and is not itself intercepted, but the `ctx.emit` seam is validated. Every op also carries a **mandatory `x-baybo-retryable` boolean** — the author's declaration that replaying the op is harmless (pure read/recompute) or not (side effects); install refuses a spec that omits it, so replay-safety is always a decision, never a default. The `true` set rides the deck view (`retryable_ops` per card) to clients, whose transports use it as the per-op replay verdict. The raw spec is also served (`GET /v1/deck/services/{uuid}/openapi.json`) for introspection.
...
**Turn/Trace boundary (explicit deviation).** The repo constraint says background execution enters Turn and Trace. Deck services are streaming residents, not discrete work items: modelling every 5-minute tick as a Turn would flood both stores with noise. The decision: **card provenance and lifecycle transitions** are recorded as trace events — install, update (`spec_hash` before → after), delete, restore, purge, start, stop, crash, quarantine — satisfying the "hot reload and tool updates leave provenance records in Trace" constraint; individual ticks and op calls are not Turns. Ops appear in the audit log when they egress with secrets.

**Governance boundary (second explicit deviation).** The repo constraint says tool/skill extensions carry source, version, hash, trust level, and capability declarations. A deck card carries `spec_hash` and an implicit source (the operator's own agent wrote it) — no version field, no trust tier, and capability declarations deliberately abolished by the trust decision above; `TrustLevel` in `crates/model` goes unused here. This is a knowing deviation, accepted with the trust model. If deck cards ever gain third-party provenance (sharing, import), the governance vocabulary must be adopted before that feature ships.
...
Creation is live: `DeckChanged` fires only after the dry-run gate has stored the first snapshot, so the card arrives on the deck **already populated** — the refetch it triggers carries the snapshot, and any loading face lasts only the iframe-boot instant — typically before the agent's chat reply lands. The successful create/update tool result carries a bounded `first_snapshot` summary: schema status, serialized byte count, JSON type, and top-level keys. The value stays on the Deck data plane instead of entering the agent transcript. The skill requires the agent to confirm that the observed shape matches the intended card before claiming success. The reply then tells the user the card is on their Deck and names its supported grid sizes; there is no deep link (the tab is one tap away). No modal and no per-card phone confirmation, consistent with the trust model.

### Client build, localization, CI

The tab label has `Localizable.xcstrings` keys (en `Deck` + the zh-Hans kanban term), plus the deck-native strings (the empty-board prompt and action, `deck.editDone`, and the delete-confirm set); the shell carries its own tiny en/zh string table for card overlays fed by a `setLanguage` bridge call (deliberately not the transcript's i18next — the shell is dependency-free vanilla TS). Packaging: the shell is a second entry in the SAME dist, so `build-app.sh`'s existing `web/dist → App/Resources/transcript/` copy ships it with no new step and `project.yml` is untouched. CI: no filter changes — the shell lives under the already-filtered `app/mobile/web`, and `crates/deck` is not an ffi dependency, so `ios-web` picks the shell up with no gate work.

## Authoring pipeline

Card authoring is **agent-or-explicit invocation and owner-channel-only**: the
builtin `deck` skill keeps `command: deck` and leaves model invocation enabled,
with `channels: [owner]`; all four `DeckCard*` tool manifests
(`List`/`Get`/`Create`/`Update`) also carry `channels: [owner]`. On the owner
surface, the skill listing advertises deck authoring so the model can pull the
skill for an ordinary-language request to create or update a persistent card;
`/deck <request>` remains the direct shortcut and injects the same body. A
telegram/tui/subagent session sees neither the skill nor the tools (they are
filtered from its LLM tool list and refused by the executor).

When the user requests a Deck card, with or without `/deck`:

1. **Skill.** For ordinary language, the model selects the advertised builtin through the `Skill` tool; for `/deck`, slash expansion injects it directly. Either path provides the bundle contract, the `ctx`/`deck` SDK surface, and worked examples deliberately spanning genres (an API-fetch quota card *and* a fetch-free machine-status card via `ctx.exec`) so the skill doesn't anchor generation to one shape.
...
3. **`DeckCardCreate(path)` — install is a dry-run gate.** The tool: (a) static-validates the manifest and spec, including mandatory response schemas and on-schema refresh params; (b) boots the service on the host — a missing `ops` export dies here; (c) invokes the refresh op once; (c2) **runs `card.html` against that snapshot** through the real client SDK (`render.rs` + `sdk/render_check.js`) and rejects a card that throws, that reads an id its markup lacks, or that displays nothing when handed its own data — the frontend half was previously read, capped and hashed and nothing more, which is how a card rendering only placeholder dashes reached a user; (d) rejects null, oversize, off-schema, and top-level `error` results — all **before** the row enables or `DeckChanged` fires. Failures (stderr, bad JSON, timeout, schema paths, or the bounded graceful-error text) return in the tool result so the agent iterates in the same turn; success stores the first snapshot, so the user's first sight is populated.
4. **Agent-visible result check.** Create/update returns schema status and a bounded structural summary of the checked first snapshot, never its value. The builtin skill requires the agent to confirm the observed JSON type and top-level keys before reporting success. Snapshot contents stay on the Deck data plane; rendered or semantic content checking remains deferred.
...
- **Schemas check declared structure, not truth.** A detailed schema catches the original class of swallowed execution errors and malformed values; it cannot prove that a syntactically valid number was computed correctly. The authoring result exposes only a structural summary, so semantic or visual content checking remains deferred.
- **Loosened validate-time caps raise per-op cost** (params 16 KB → 1 MB and results up to the 5 MB snapshot cap over author-controlled JSON Schema on the `fancy-regex` ECMA engine) — but ReDoS/recursion stay blunted (`$ref` rejected, `default-features = false`, serde depth-128, fancy-regex backtrack limit), so this is a cost ceiling, not an unbounded-time hole.

## Deferred and open items

- **Web parity** (`app/web` `/deck` page) — deferred; requires extracting the shell from `app/mobile/web`.
