...
3. **`DeckCardCreate(path)` — install is a dry-run gate.** The tool: (a) static-validates the manifest and spec, including mandatory response schemas and on-schema refresh params; (b) boots the service on the host — a missing `ops` export dies here; (c) invokes the refresh op once; (d) rejects null, oversize, off-schema, and top-level `error` results — all **before** the row enables or `DeckChanged` fires. Failures (stderr, bad JSON, timeout, schema paths, or the bounded graceful-error text) return in the tool result so the agent iterates in the same turn; success stores the first snapshot, so the user's first sight is populated.
