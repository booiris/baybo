...
**Scratch lifecycle.** `ctx.exec` runs in a per-card cwd under `state/deck-scratch/<card_id>`, reaped at purge (above). The dry-run gate runs its throwaway service in `state/deck-scratch/gate-<uuid>`, removed when the gate finishes; a crash mid-gate strands one, so `DeckManager::boot` reaps every `gate-*` entry under the scratch root — none can be in flight at boot.
