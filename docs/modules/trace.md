...
The `trace` crate defines domain types for call-chain tracing (`SessionTrace`, `TraceNode`, `TraceSpan`, `SpanHandle`, `SpanInput`, `SpanResult`, `ExecutionProvenance`, `ForkRecord`, `TraceFilter`) and provides tree and fork utility functions.
...
Branching (via `aura_trace::fork::fork_from`) creates a new branch below a target node without overwriting the original chain. Both branches are preserved for audit and comparison.

## Constraints

- Types crate with tree/fork utilities — no collector logic
- Depends on `job` (for `OperationKind`)
