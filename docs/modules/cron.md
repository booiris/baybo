...
1. **Waiter.** When the Router dispatches a one-shot fire it also spawns a waiter (`actor/router/cron.rs`), subscribed to the `TurnLifecycle` bus **before** the trigger is sent — the fire's terminal event is published from inside its own turn, so a later subscription could miss it. The waiter also watches the fire actor's token, so an actor that dies before opening a turn still produces a notification (reconciled against the store, then reported as a failure) rather than nothing. It runs **even when the trigger cannot be handed to the actor at all** (a closed mailbox during a shutdown race): the scheduler has already recorded that execution as dispatched, so nothing else would ever retry it — the Router trips the token instead, and the waiter reports the fire as failed.
...
- **Push** — the fire's `Cron` turn completes with a reply-less `TurnOutput::Structured` result, so its `Completed` edge carries no reply ordinal and `PushDispatcher` skips it (`gateway::push` returns early on `reply_ordinal: None`).
...
## Fires run unattended

A fire's turn — and every in-process descendant it spawns — runs unattended,
carried by the field-less `InheritedToolContext` marker (see
[`agent.md`](agent.md)'s approval-gate wiring for the mechanism). Any tool call
with an approval-gated access not already covered by session approvals is
denied immediately instead of parking on a prompt nobody can answer. MCP tools
declare no approval-gated accesses (the interim rule in
[`todo/mcp-tool-approval.md`](../todo/mcp-tool-approval.md)), so they are
usable in fires exactly as in chat. Nothing about a fire ever seeds
`SessionState::approved_resources`; a user reply in a recurring fire's
conversation or an independently resumed child gets ordinary interactive
approval behavior.
...
At boot, `Router::run` re-drives that scan before any live traffic. The origin actor appends with the source-event key `cron-execution:<execution_id>`; a unique partial index on `(session_id, source_event_id)` claims the key in the same statement as the transcript row. A replay therefore returns the original ordinal without opening another notification turn, appending another row, or dispatching another live message, then stamps `notified_at` so the scan converges.
...
notification turn and external channel/push dispatch are not in the same
...
| `turn`    | `TurnInput::Cron` marks a fire; `TurnInput::CronNotification` marks the (inference-free) delivery of a one-shot's result, whose `Completed { reply_ordinal }` edge drives push |
