...
### Per-session model selection and spawn-time reasoning effort

Each session can pin its own `baybo.json` LLM entry via `session.state.last_llm` (`None` ⇒ follow `default-llm`, so an un-switched session keeps tracking global default changes). The pin flows into the loop's `initial_llm`: at a cold spawn / post-eviction hydration, `Router::handle_incoming` resolves `crate::actor::router::user_input::resolve_spawn_llm(agent_profile_store, session)` and passes the result to the actor spawner (the `ActorSpawner` closure); for a **live** actor, `AgentMessage::SetModel { llm }` (Trigger-tier, so it lands at a turn boundary — never mid-turn) re-pins the loop in place via `AgentLoop::set_initial_llm`. Either way the swap takes effect on the **next** turn: `AgentLoop::refresh_active_llm` re-resolves `initial_llm` against the hot-swappable `LlmClientPool` at the top of every turn — the same hook that absorbs config hot-reloads — swapping the client and context-window budget when the resolved entry changes. A stranded pin (entry later removed from config) degrades safely: `LlmClientPool::resolve` falls back to the default with a `warn!`.

`resolve_spawn_llm` returns `SpawnLlmChoice { initial_llm, reasoning_effort }` (`crate::actor::router::SpawnLlmChoice`), the `ActorSpawner`'s second argument alongside the session. For an unbound session (`session.state.agent_id` is `None`) it's just `session.state.last_llm` with `reasoning_effort: None`; for a bound one it fetches the profile — via `AgentProfileStore::get`, live, not cached on the `Router` — even when `last_llm` is already set, because the session pin only ever supplies the *model*: `initial_llm = last_llm.or(profile.llm)`, but `reasoning_effort = profile.reasoning_effort` unconditionally. A missing/deleted profile or a store error both degrade to `session.state.last_llm` + `reasoning_effort: None` with a `warn!`, never a spawn failure. `SpawnLlmChoice` lands on `AgentLoopConfig.reasoning_effort`, and `AgentLoop::call_llm` is the **only** place that reads it into `ChatRequest.reasoning_effort` — title generation, the progress observer, and background compression build their own `ChatRequest`s with `reasoning_effort: None` regardless. Liveness matches the LLM pin's spawn-time half exactly (a profile edit lands on the next actor spawn/hydration) but **not** its live-repin half: `SetModel` only ever carries `llm: Option<LlmEntryName>`, so an in-flight actor's reasoning effort is fixed for its lifetime. See [`llm.md`](llm.md#reasoning-effort-bridge) for how the value reaches the provider and [`agent-profiles.md`](agent-profiles.md#content-resolution-identity-snapshots-content-follows-live) for the full profile-side story, including the `allowed_models` set that `PUT /v1/chat/sessions/{id}/model` enforces on a *new* explicit pin (a clear always bypasses it) — enforcement stays at switch time, orthogonal to `resolve_spawn_llm`'s precedence; the resolver does consult `allowed_models` for the bound arm, but only to `warn!` when a historical pin now sits outside the set, tolerating it rather than rewriting or rejecting it.

Persistence and the live re-pin are deliberately **split** to avoid a lost-update race. `last_llm` is a **flat `sessions` column**, not a JSON-blob field — exactly like `hidden` — written only by the targeted `SessionStore::set_last_llm` and omitted from `save`'s `DO UPDATE`, so a concurrent `touch` (which is a full-blob `get` + `save` fired on every inbound message) can't clobber it; `get` patches `Session.state.last_llm` from the column on read. The chat `PUT /v1/chat/sessions/{id}/model` validates the name against the pool, then (1) **persists** via `set_last_llm` synchronously — authoritative for any later spawn, and a storage failure surfaces as an error rather than a false 200 — and (2) routes `SetModel` to re-pin the live actor **in memory only** (the gateway holds an `AgentSupervisor` clone for this reach-the-live-actor hop, the same way `/stop` reaches one). `SetModel` does not itself persist. Subagent spawns are the other `initial_llm = Some(...)` path, pinning via `model_tier` instead — they build `SpawnLlmChoice { initial_llm: llm, ..Default::default() }` (`crates/agent/src/runtime/subagent_spawner.rs`), so `reasoning_effort` is always `None`, since subagent sessions are never agent-bound.
...
| Module | Role |
|--------|------|
| `llm` | `AgentLoop` initiates model calls, including the profile-driven `reasoning_effort` ask on the main-loop `ChatRequest` (see [Per-session model selection and spawn-time reasoning effort](#per-session-model-selection-and-spawn-time-reasoning-effort)) |
