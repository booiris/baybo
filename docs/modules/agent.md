...
1. System-prompt assembly lives in [`aura_context::prompts::soul`](context.md) (`assemble_from_workspace` reads `profile/{SOUL,USER,IDENTITY}.md` and frames them with TOP/TAIL hints). `ContextManager` holds the resolved `system_prompt` (a subagent profile overrides it at construction) and owns both the seed — `AgentLoop::ensure_system_prompt` reads it via `context_manager.system_prompt()` — and the reseed-after-compaction. Mid-session profile writes are **not** picked up except on compaction (which re-reads the workspace); see [`docs/todo/profile-hot-reload.md`](../todo/profile-hot-reload.md).
