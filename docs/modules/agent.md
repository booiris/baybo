...
`AgentMessage::CronResultReady` is handled at a turn boundary with **no inference** (`handle_cron_result_ready`): dedup against `session.state.delivered_cron_executions`, un-hide the conversation if the user had removed it, read the fire's reply row, append it as a `MessageSource::CronNotification` assistant row framed with a scheduled-task header, dispatch it, and resolve the ledger. It opens a `JobInput::CronNotification` job purely so the append rides a `Completed { reply_ordinal }` edge — that is what the push dispatcher previews from. Every outcome notifies, including failure and an empty reply: a scheduled task that silently evaporates is the failure mode this design exists to prevent. See [`cron.md`](cron.md) for the full delivery contract.

Background subagent results arrive as `AgentMessage::BackgroundJobFinished`, are buffered on `session.state.pending_background_results` (dedup by `handle_id`, cap 64 drop-oldest), and — once no higher-priority message is queued — drained into their own autonomous `SubagentNotification` agent-loop turn (same main path / system prompt + toolset, so the prompt cache holds; the model proactively reports to the user, and an empty reply is suppressed).

Delivery is **ledgered, append-once, forward-only**. The drain persists the synthetic XML prompt (built by `baybo_context::prompts::subagent::build_notification_content`) as a hidden agent-context row via `AgentLoop::append_subagent_notification` — an ordinary `session_messages` append, so the window/log lockstep that the trace input markers and the compression fast-path rely on holds by construction — then records `{content, prompt_ordinal, attempts}` on `session.state.pending_notification_turn` (the delivery ledger, durable session state like the buffer itself) and runs the turn. Nothing rolls back: a failed attempt's partial rows are real history that the persisted prompt row explains, and the failure only bumps `ledger.attempts`. Retries fire from the run loop's timed backoff arm only (never from the post-message drain — the intervening turn's request already carried the prompt row); each retry appends a small persisted **cue row** (`build_retry_cue`) rather than re-building the prompt — the cue restores a user-side request tail (a cancelled attempt's salvage leaves an assistant row at the tail, and a request ending on an assistant message is provider prefill, rejected outright by Anthropic with extended thinking on), un-buries the prompt, and makes a blank retry reply a genuine judgment. If a compaction superseded the prompt row (compaction re-inserts every kept row at fresh ordinals, so the recorded ordinal dangles after any compaction), the retry re-appends the prompt from the ledger's frozen `content` and re-stamps the ordinal. At `NOTIFY_TURN_MAX_ATTEMPTS` the ledger clears and delivery degrades to **passive** — the prompt row is durable in the transcript, so the next real turn's model reads and reports it; the cap exists because each retry's state persist bumps `last_active`, so an ever-failing turn would otherwise pin the actor resident forever (store-error retry paths count against the same cap). A **user turn completing over an open ledger settles it** the same passive way, at turn end in `handle_merged_user_turn`: that turn's request already carried the prompt row, so a later proactive retry would duplicate the report — and the settle keys off the actor's own knowledge that the turn completed, because transcript rows can't carry that fact (a failed turn's salvage row is indistinguishable from a delivered final reply). Crash stance (deliberate): a crash mid-turn re-drives from the durable ledger on rehydration and may deliver a duplicate report — a duplicate beats a lost notification, the same direction the cron delivery ledger chose. The ActorStop final state write (which heals a ledger whose post-delivery clear failed to persist) leaves `last_active` at the store's authoritative value: the router touches it straight in the store on every inbound message, so the actor's in-memory copy is typically older, and the chat list orders by this column.
...
| `job` | Owns `Job`, `JobStatus`, `JobInputKind` / `JobInput` (+ `Job.origin`), and `JobLifecycle` (persistence orchestrator + cancellation registry + lifecycle-event bus); the `JobStore` trait lives in `baybo-store` and this crate owns the `Job` ↔ `JobRow` conversions. Agent constructs and shares one `JobLifecycle` across the loop, router, supervisor, and subagent wait routine |
