...
`model_list` is edited over HTTP as a whole SET (`PUT /v1/llm/models/{name}/model-list`), which is what lets a model be *removed* — before that route existed the list could only grow, because its only writers were the file, the setup wizard, and `default_spec_mut()` materialising a departing default. The handler carries each surviving id's spec across, so replacing the set never destroys an override. Per-model overrides for a non-default model remain file-only: `PUT /v1/llm/models/{name}` addresses `model`'s spec and nothing else.
...
- **Most providers** get the entry's **own** model. That is a behavioural no-op — resolution step 1 then hands back the entry's default client, exactly as an unset field would — written purely so the knob is visible: the field is `skip_serializing_if = "Option::is_none"`, so leaving it unset means an operator has no way to learn from their own config file that the auxiliary calls can be moved to a cheaper model. Editing it to a cheaper entry model is the intended next step.
- **`openai-subscription`** gets a real one: `gpt-5.6-terra`, the middle of the gpt-5.6 family's three tiers, so the auxiliary calls don't spend the plan's quota on whatever flagship the operator picked for chat. Because `lite_model` must name one of the entry's own models (validated against `models()`, above), the wizard also writes `gpt-5.6-terra` into `model_list` — the one case where a wizard-created entry ships a non-empty list. The seed is gated on the live catalog actually offering the model, so a plan that doesn't serve it, or a failed model-discovery call, falls back to the no-op form rather than writing a dangling reference. Note this is a *quota and latency* choice, not a cost one: every call on this provider records $0 (§"Billing" in [`llm-openai-subscription.md`](llm-openai-subscription.md)).

A `lite_model` is never added to the entry's pinnable set *by resolution*: it is what the runtime picks for itself, not a menu item. An operator who wants it in the chat picker lists it in `model_list` as well, and it is then built once and serves both roles. The wizard's `openai-subscription` seed above lands in exactly that state — it has to list the model to satisfy validation, so `gpt-5.6-terra` is pinnable there as a side effect, not because the lite role implies it.
...
| `permission` | `baybo_tools::builtin::PermissionMode`                   | Permission policy: `auto` (default), `manual`, or `free` (waives the approval gate for `Bash`, `Write` and `Edit`) (`open`/`none` accepted as legacy aliases for `free`). Hot-reloadable through `LivePermissionMode`; see [`../permission.md`](../permission.md). |
...
| `trace`    | `LlmCall` spans record which model answered (`model_id`, `provider`) and at what thinking level (`reasoning_effort`) on their begin payload (`LlmCallBegin`, `baybo-trace::span`). Config load/change itself is not traced. |
