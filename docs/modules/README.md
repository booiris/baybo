...
sandbox   ──► (no internal deps; OS sandbox runner consumed by agent and deck)
...
deck      ──► model, store, security, sandbox, tools (card bundles + dry-run install gate + supervised sandboxed bun service runtime; owns the DeckCardCreate/DeckCardUpdate tools; gateway consumes DeckManager)
