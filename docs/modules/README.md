...
- **memory** — User-scoped memory store: `MemoryStore` trait and `MemoryManager` business-logic facade (list/search/store/delete/importance, per-user eviction). Domain types (`MemoryEntry`, `MemoryCategory`) live in `model`. Used by the admin REST surface only; the agent loop has no automatic recall or auto-store path.
