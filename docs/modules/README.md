...
- **[project](project.md)** — `baybo-project`: kanban boards. Owns the board's write surface (`ProjectManager`), the pure rule modules behind it (the two run predicates, comment delivery, mentions, stage barriers, the daily budget, timeline diffing), the per-issue git worktree layer, the `TimelineApprovalGate`, and the six board tools (`IssueList`/`Get`/`Create`/`Update`/`Comment`, `ProjectAgentCreate`) — which take no `project_id`, reading their board off the session's `TriggerSource` instead. Like `cron`/`skills`/`subagent`/`task`/`deck` it hosts its own `Tool` impls over `baybo-tools`; the `ProjectStore` trait lives in `store`, the sqlite impl in `storage`, the run executor in `agent`, and the REST/WS surface in `gateway`. One enqueue chokepoint gates every run on liveness, dedupe and budget.
