...
- **agent** — Assembly layer: Actor, AgentLoop, ToolExecutor, observability facades (`JobLifecycle` for the job state machine, `SpanRecorder` for Step/Span/SpanEvent writes), cost management (`CostManager` as a `TraceEventStream` subscriber, with `CostGuardError` for limit breaches), plus all domain managers (MemoryManager, SecretVault, SecurityGateway, CronScheduler). Re-exports `SessionManager` from `aura-session` for callers that pull session lifecycle through this crate. Bridges cron domain types and storage row types.
