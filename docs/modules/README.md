...
- **[tui](tui.md)** — Interactive terminal UI (`aura-tui`). Ratatui + Crossterm frontend driven by an HTTP+SSE `GatewayTransport` client of `aura-gateway`; no local manager graph, no workspace singleton. Hosts `TuiAdapter`, `GatewaySlashHandler`, `GatewayDashboardProvider`, and the `InputHistoryStore` trait. Depends on `aura-channels` for shared trait definitions only.
