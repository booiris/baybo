...
- **store** — `baybo-store`: the persistence **ports** crate. Owns the shared `StorageError` and every `*Store` trait contract (`SessionStore`, `SessionSummaryStore`, `SessionFolderStore`, `TaskStore`, `JobStore`, `TraceStore`, `CostStore`, `SecretStore`, `CronStore`, `BlobStore`, `ChannelPairingStore`, `ChannelSessionStore`, `ChannelBotStore`, `DeviceStore`, `SkillRiskStore`, `AgentProfileStore`) plus the row/DTO types those traits exchange. Depends only on `model`; `baybo-storage` is the sqlite adapter that implements the traits. `JobStore` / `TraceStore` trade in row DTOs (`JobRow` / `JobTransitionRow`, `StepRow` / `SpanRow` / `SpanEventRow` — a queryable key plus the serialized entity in `data`) so the trait can sit here as a leaf while the rich `Job` / `Step` / `Span` types and their state-machine / recorder logic stay in `job` / `trace`, which own the `to_row` / `from_row` conversions.
...
- [agent-profiles.md](agent-profiles.md) — User-managed chat personas (`AgentProfile`): DB-backed profiles bundling name, description, avatar (blob), system prompt (`NULL` = Soul), framework (`baybo`/`claude`/`codex`), an optional LLM pin, an `allowed_models` switch-target set (empty = unrestricted), and a `reasoning_effort` override, with a locked built-in `baybo` row. A web chat session binds to one at creation (`sessions.{agent_id, agent_framework}`, write-once), and the bound profile then drives that session's system prompt, LLM precedence, model-switch set, reasoning effort, and skill overlay (`<workspace>/agent-skills/<agent_id>/`) live — see "Session binding" in the doc. `claude`/`codex` profiles stay management-only until the external-agent chat leg lands. Spans `model` (id + framework types + `ReasoningEffort`), `store`/`storage` (`AgentProfileStore` + `agent_profiles` table + `sessions` binding columns), `gateway` (`/v1/agents` CRUD + `POST /v1/chat/sessions` binding + the `allowed_models` switch-time check on `PUT …/model`), `context`/`agent`/`skills`/`memory`/`llm` (live consumption), and the web **Agents** page + chat picker/chips + ModelPicker filtering. Distinct from `subagent`'s `SubagentProfile`.
