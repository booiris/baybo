...
- **agent** — Assembly layer: Actor, AgentLoop, ToolExecutor, observability facades (`JobLifecycle` for the job state machine, `SpanRecorder` for Step/Span/SpanEvent writes), cost management (`CostTracker` as a `TraceEventStream` subscriber, `CostGuard`), plus all domain managers (MemoryManager, SecretVault, SecurityGateway, CronScheduler). Re-exports `SessionManager` from `aura-session` for callers that pull session lifecycle through this crate. Bridges cron domain types and storage row types.
- **[self_improvement](self-improvement.md)** — Side-channel agent flow that runs after a complex user-chat job completes (`iterations > 8`, default-on). Subscribes to `JobLifecycle::terminal_events`, spawns a fresh `TriggerSource::System { reason: SelfImprovement }` session through `Router::handle_system_trigger` (symmetric with cron), and runs a two-phase Survey → Justify → Write loop with a tightly scoped tool ceiling (`MemoryWrite`, `SkillCreate`, `MemoryList`, `SkillList`). Lives inside `aura-agent` as a module.
