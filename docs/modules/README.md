...
- [agent-profiles.md](agent-profiles.md) — User-managed chat personas (`AgentProfile`): DB-backed profiles bundling name, description, avatar (blob), system prompt (`NULL` = Soul), framework (`baybo`/`claude`/`codex`), and an optional LLM pin, with a locked built-in `baybo` row. A web chat session binds to one at creation (`sessions.{agent_id, agent_framework}`, write-once), and the bound profile then drives that session's system prompt, LLM precedence, and skill overlay (`<workspace>/agent-skills/<agent_id>/`) live — see "Session binding" in the doc. `claude`/`codex` profiles stay management-only until the external-agent chat leg lands. Spans `model` (id + framework types), `store`/`storage` (`AgentProfileStore` + `agent_profiles` table + `sessions` binding columns), `gateway` (`/v1/agents` CRUD + `POST /v1/chat/sessions` binding), `context`/`agent`/`skills`/`memory` (live consumption), and the web **Agents** page + chat picker/chips. Distinct from `subagent`'s `SubagentProfile`.
