...
- [agent-profiles.md](agent-profiles.md) — User-managed chat personas (`AgentProfile`): DB-backed profiles bundling name, description, avatar (blob), system prompt (`NULL` = Soul), skill/tool allow-lists (`NULL` = inherit all), framework (`baybo`/`claude`/`codex`/`gemini`), and an optional LLM pin, with a locked built-in `baybo` row. Spans `model` (id + framework types), `store`/`storage` (`AgentProfileStore` + `agent_profiles` table), `gateway` (`/v1/agents` CRUD), and the web **Agents** page. v1 is management-only — no runtime consumer reads profiles yet; distinct from `subagent`'s `SubagentProfile`.
