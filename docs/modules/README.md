...
5. [trace.md](trace.md)
6. [storage.md](storage.md) → [janitor.md](janitor.md) → [pairing.md](pairing.md) → [deck.md](deck.md) → [project.md](project.md) → [agent.md](agent.md) → [setup.md](setup.md) → [bootstrap.md](bootstrap.md) → [cli.md](cli.md) → [gateway.md](gateway.md) → [tui.md](tui.md)
...
- **store** — `baybo-store`: the persistence **ports** crate. Owns the shared `StorageError` and every `*Store` trait contract (`SessionStore`, `SessionFolderStore`, `MessageSearchStore`, `TaskStore`, `TurnStore`, `TraceStore`, `CostStore`, `SecretStore`, `CronStore`, `BlobStore`, `ChannelPairingStore`, `ChannelSessionStore`, `ChannelBotStore`, `DeviceStore`, `SkillRiskStore`, `AgentProfileStore`, `DeckCardStore`, `ProjectStore`) plus the row/DTO types those traits exchange. Depends only on `model`; `baybo-storage` is the sqlite adapter that implements the traits. Session active reads use the narrow `ActiveMessageRow { ordinal, message }`; historical trace reads use `StoredMessage` with supersede and timestamp metadata. `TurnStore` / `TraceStore` trade in row DTOs (`TurnRow`, `StepRow` / `SpanRow` / `SpanEventRow` — a queryable key plus the serialized entity in `data`) so the trait can sit here as a leaf while the rich `Turn` / `Step` / `Span` types and their state-machine / recorder logic stay in `turn` / `trace`, which own the `to_row` / `from_row` conversions.
...
- **[project](project.md)** — `baybo-project`: kanban boards. Owns the board's write surface (`ProjectManager`), the pure rule modules behind it (the two run predicates, comment delivery, mentions, stage barriers, the daily budget, timeline diffing), the per-issue git worktree layer, the `TimelineApprovalGate`, and the seven board tools (`IssueList`/`Get`/`Create`/`Update`/`Comment`/`Merge`, `ProjectAgentCreate`) — which take no `project_id`, reading their board off the session's `TriggerSource` instead. Like `cron`/`skills`/`subagent`/`task`/`deck` it hosts its own `Tool` impls over `baybo-tools`; the `ProjectStore` trait lives in `store`, the sqlite impl in `storage`, the run executor in `agent`, and the REST/WS surface in `gateway`. One enqueue chokepoint gates every run on liveness, dedupe and budget.
...
- **session** — Owns the session business logic: the `SessionManager` facade (CRUD, idle-session listing for the actor reaper (`idle_sessions` — never deletes rows), transcript persistence helpers, chat-list folders) and `SessionError` (with `From<StorageError>`). The `SessionStore` / `SessionFolderStore` traits and their `ActiveMessageRow` / `StoredMessage` / `SessionFolderRow` row types now live in `store`; session domain types (`User`, `ChannelType`, `Session`, `SessionState`, `TriggerSource`, `Lineage`) live in `model`; `baybo-storage` provides the sqlite implementations. A `Session` is the top of one trace tree (1 trace = 1 session); subagent spawn creates new sessions linked through `Lineage`.
...
- **storage** — The sqlite **adapter**: implements every `*Store` trait from `store` over a single sqlite backend and bundles the impls in `Store` for DI. All trait contracts + their row types (incl. `RiskVerdict` / `RiskLevel`, `ChannelPairingRow` / `PairingStatus`, `TurnRow`, `StepRow` / `SpanRow` / `SpanEventRow`) now live in `store`; consumers import them from `baybo_store` directly (`baybo-storage` does **not** re-export them — it exposes only the `Store` DI bundle, the `sqlite` module, and the `retry` helper). Normal deps are `store` (trait contracts + row types), `model` (domain types), and `workspace` (path constants — a leaf with no `baybo-*` deps of its own, so it introduces no domain edge); no domain crate (`session` / `turn` / `trace` / `cost` / `cron` / `memory` / `security`). `turn` / `trace` remain only as `dev-dependencies`, so the round-trip tests can build the rich types.
...
- [agent-profiles.md](agent-profiles.md) — User-managed chat personas (`AgentProfile`): DB-backed profiles bundling description, avatar (blob), framework (`baybo`/`claude`/`codex`), and an optional LLM pin, with a locked built-in `baybo` row. Neither the name nor the prompt is a column — both are files in the agent's own persona directory under `<workspace>/personas/`, and skills are shown read-only live from the registry. Spans `model` (id + framework types), `store`/`storage` (`AgentProfileStore` + `agent_profiles` table), `gateway` (`/v1/agents` CRUD plus the per-board roster at `/v1/projects/{id}/agents`), `agent` (a session binds to a profile at creation and the spawner reads its LLM pin), and the board page's `TeamStrip`. Distinct from `subagent`'s `SubagentProfile`.
...
storage   ──► store, model, workspace (implements every *Store trait from the ports crate; exposes the Store DI bundle + sqlite + retry; sole backend: sqlite)
...
session   ──► model, store (owns SessionManager + SessionError; the SessionStore / SessionFolderStore traits live in store)
...
project   ──► model, store, tools, turn, workspace (kanban boards: the ProjectManager write surface, the pure run/comment/stage/budget rules, per-issue git worktrees, the TimelineApprovalGate, the IssueRunStopper port that stops the turn under a live run, and the seven board tools; agent executes the runs it records, gateway serves the board)
agent     ──► model, llm, tools, subagent, skills, skills-assessor, workspace, context, session, trace, turn, memory, cost, cron, project, security, sandbox, store, storage, channels
gateway   ──► agent, channels, config, context, cron, cost, deck, llm, model, pairing, project, query, security, session, skills, storage, store, tools, trace, turn, workspace
