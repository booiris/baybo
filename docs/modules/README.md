...
project   ──► model, store, tools, workspace (kanban boards: the ProjectManager write surface, the pure run/comment/stage/budget rules, per-issue git worktrees, the TimelineApprovalGate, and the six board tools; agent executes the runs it records, gateway serves the board)
