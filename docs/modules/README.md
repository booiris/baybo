...
- **store** — `aura-store`: the persistence **ports** crate. Owns the shared `StorageError` and every `*Store` trait contract (`SecretStore`, `MemoryStore`, `CronStore`, `CostStore`, `SessionStore`, `SessionSummaryStore`, `BlobStore`, `ChannelPairingStore`, `ChannelSessionStore`, `ChannelBotStore`, `SkillRiskStore`) plus the row/DTO types those traits exchange. Depends only on `model`; `aura-storage` is the libsql adapter that implements the traits. (`JobStore` / `TraceStore` stay in `job` / `trace` — their row types carry domain logic that does not belong in a leaf crate.)
...
- **storage** — The libsql **adapter**: implements every `*Store` trait from `store` over a single libsql backend and bundles the impls in `Store` for DI. All trait contracts + their row types (incl. `RiskVerdict` / `RiskLevel`, `ChannelPairingRow` / `PairingStatus`) now live in `store`; `aura-storage` re-exports them for back-compat. Depends on `store` + `model`, plus `job` + `trace` (only because `JobStore` / `TraceStore` stayed in those domain crates) — no longer on `session` / `cost` / `cron` / `memory` / `security`.
