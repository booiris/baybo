...
| `workspace`  | `gc [--days <N>] [--apply] [--yes]`                                                                       | `WorkspacePaths` over `<workspace>/work` (see [`workspace.md`](workspace.md)) | report-only by default: classifies top-level `work/` entries into git clones, stale (newest in-tree mtime older than `--days`, default 14), empty dirs, and `work/tmp` contents (first match wins), with du-style sizes that never follow symlinks. Protection is a policy, not an allowlist: **every dot-prefixed top-level entry** is never listed — sandboxed Bash runs with `HOME` = the work dir, so `.gitconfig`, `.config/`, `.npm/`, `.cargo/`, `.claude`, `.claude.json`, `.codex`, and baybo's own `.uv/`/`.fonts/`/`.baybo-tool-spills/` are live HOME state whose mtimes never refresh on read — plus the external-agent roots and `tmp` itself; non-dotted agent-authored names are ordinary candidates the report surfaces for the user to decide on. `--apply` prints the report and prompts on stderr, then deletes per category after a y/N stdin confirm (`--yes` skips; it requires `--apply` and is rejected without it); stdout carries only the final summary (JSON under `--json`). A symlinked entry is removed as a link, never traversed. Shell-only — the whole family is rejected in slash and dropped from the slash menu. | shipped                                           |
