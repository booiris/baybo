...
| `config`     | `get <path>` · `set <path> <value>` · `unset <path>`                                                      | `AuraConfig`                                                                 | `set`/`unset` write `aura.json`; take effect after restart (hot-reload deferred — see `config.md`) | deferred — needs JSON-pointer path setter         |
...
| `tools`      | `test <name> [--args <json>]`                                                                             | `ToolExecutor`                                                               | `test` mutates observably                                                                          | deferred — needs `test_execute`                   |
...
| `llm`        | `models` · `probe`                                                                                        | `LlmProviderRegistry` / `LlmClient`                                          | `probe` issues a minimal request                                                                   | deferred — needs `list_models` / `probe`          |
| `workspace`  | `show`                                                                                                    | `WorkspaceManager`                                                           | read-only                                                                                          | shipped                                           |
| `workspace`  | `set-identity`                                                                                            | `WorkspaceManager` / `Soul`                                                  | `set-identity` mutates                                                                             | deferred — needs `write_identity_file`            |
| `session`    | `list` · `show <id>` · `history <id>` · `kill <id>`                                                       | `SessionManager`                                                             | `kill` mutates                                                                                     | deferred — needs list / history / delete          |
| `job`        | `list [--status]` · `show <id>` · `cancel <id>`                                                           | `JobManager`                                                                 | `cancel` mutates                                                                                   | deferred — needs list / get / cancel              |
| `trace`      | `list` · `show <id>` · `export <id> [--out <path>]` · `snapshot <session-id>`                             | `TraceCollector` / `SessionTrace`                                            | `snapshot` mutates; `export` is a local write                                                      | deferred — needs list / get / export / snapshot   |
| `cron`       | `list` · `add` · `show <id>` · `rm <id>` · `enable <id>` · `disable <id>` · `run <id>` · `runs --id <id>` | `CronScheduler` / `CronStore` (via agent)                                    | `add`/`rm`/`enable`/`disable`/`run` mutate                                                         | deferred — user-less `list`, `get`, `trigger_now` |
| `memory`     | `list` · `search <query>` · `promote <id>` · `clear --session <id>`                                       | `MemoryManager`                                                              | `promote`/`clear` mutate                                                                           | deferred — needs list / search / promote / clear  |
| `security`   | `audit` · `leaks check <file>`                                                                            | `SecurityGateway` / `LeakDetector` / `SecretVault`                           | read-only                                                                                          | deferred — needs `audit` + `check_file`           |
