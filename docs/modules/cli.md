...
| `external-agent` | `status` · `setup` · `disable`                                                            | `baybo-agent::external_agent` CLI backends (Claude Code / Codex)     | Both kinds are `enabled` by default and registered at boot when their binary is on `PATH`, so these commands are for adjusting that, not for turning the feature on. `status` re-probes each kind offline (read-only); `setup` is an interactive wizard that probes a binary path and writes the resolved **absolute** `external_agents.<kind>.binary_path` to `baybo.json` (empty input = PATH lookup, still recorded as a concrete path); `disable` is a multi-select that flips `external_agents.<kind>.enabled = false` for the checked kinds (no-op success when nothing is enabled) — the way to withhold an installed backend | shipped                                           |
...
| `gateway`    | `start` · `install [--system] [--exec-start <p>] [--run-as <user>]` · `enable` · `restart` · `disable` · `uninstall` · `status` · `token {show, rotate}` | `baybo-gateway` installer + `AdminToken`                                      | `start` runs the long-lived server; `enable` enables autostart and starts it; `restart` restarts it; `disable` disables autostart and stops it; `install`/`uninstall` and `token rotate` also mutate; `status`/`token show` are read-only | shipped (intercepted in `crates/baybo/src/main.rs` before dispatch, runs in `crates/baybo/src/gateway_cmd.rs`) |
...
| `device`     | `pair [--proxy-url <url>] [--push-url <url>] [--remote-api-key <key>]` · `list [--approved]` · `revoke <device-id>` | `baybo_pairing::DevicePairingService` (device store) | `pair`/`revoke` mutate; `pair` records independent relay and push endpoints, while `remote_api_key` is relay-only; the interactive mutual-confirm flow is terminal-only (the whole family is rejected in slash) | shipped |
