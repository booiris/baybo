...
| `memory`     | `status` · `configure` · `test` · `set-key` · `disable`                                                   | `aura-memory` backends + `SecretVault` (per-provider `memory.<provider>.api_key`) | `status` / `test` are read-only (test runs the backend's startup health probe); `configure` is interactive (provider + per-field prompts) and persists to `aura.json` + vault; `set-key` writes the API key under `memory.<provider>.api_key`; `disable` flips `provider = noop` and clears `extra`. Memory config is **not** hot-reload, so each mutating command prints a restart hint. | shipped                                           |
