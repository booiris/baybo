...
| `channel`    | `list` · `add <channel-type>` · `remove <channel-type> <bot-id>` · `bots <channel-type>`                 | `ChannelRegistry` + channel-bot roster                                       | `add`/`remove` mutate; `list`/`bots` are read-only                                                 | shipped                                           |
