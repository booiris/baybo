...
### CLI subcommands (under `aura llm`)

| Command | What it does |
|---|---|
| `aura llm auth login --provider openai-subscription` | Browser PKCE: opens `https://auth.openai.com/oauth/authorize?...`, runs a one-shot HTTP listener on `127.0.0.1:1455` for the callback, exchanges the code, persists the bundle. |
| `aura llm auth login --provider openai-subscription --device-code` | Device-code variant for headless boxes (`/api/accounts/deviceauth/usercode` + polled `/deviceauth/token`). |
| `aura llm auth status --provider openai-subscription` | Prints whether a token exists, account email, plan type, expiry. Also prints which endpoint the token is used against (TOS transparency, see Security). |
| `aura llm auth logout --provider openai-subscription` | Loads the bundle, calls `/oauth/revoke` (RFC 7009, best-effort — logs but doesn't fail the command), then deletes the vault entry. **Vault delete failure is fatal**: telling the user "logged out" while a usable token is still at rest would be a security lie. The human-readable output reflects whether the server-side revoke actually succeeded so the operator knows whether the token is still valid upstream. |

These live in `crates/cli/src/commands/llm.rs` (extending the existing `aura llm` family).
...
|---|---|
| Vault returns None | `Config("openai-subscription: not signed in — run `aura llm auth login --provider openai-subscription`")` |
...
- **User-facing transparency**: `aura llm auth status` shows three distinct states: default-allowed, allowlisted-non-default, and unsafe-override-active. The third one is unmistakable in the human output.
