...
- **ModelPicker filtering** (`app/web/src/pages/ChatPage.tsx`, `filterAllowedModels` in `app/web/src/pages/chat/modelFilter.ts`): for a session bound to an agent whose `allowed_models` is non-empty, the model-switch picker is filtered to just those entries — an empty set is treated as "unrestricted" and leaves every configured model visible, mirroring the server's own semantics. If the session's currently-effective model falls outside the set (e.g. the profile was tightened after the session already switched), the picker still shows it rather than hiding it, flagged with a "(not in allowed set)" sublabel, so the user can see and change an out-of-policy pin instead of it silently vanishing. The agent's `allowed_models` rides the same `GET /v1/agents` fetch the picker and chips already use — no new endpoint, no new WS frame.
