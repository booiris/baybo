...
pub struct AgentProfileRow {
    ...
    pub allowed_models: Vec<LlmEntryName>, // empty = unrestricted switch target set
    pub reasoning_effort: Option<ReasoningEffort>, // None = follow the LLM entry's own value
...
| `allowed_models` (`[]`) | unrestricted — a bound session may switch to any pool entry, today's behavior | restricts `PUT /v1/chat/sessions/{id}/model` to naming a set member |
| `reasoning_effort` | follow the LLM entry's own configured value | overrides it on every main-loop request of a bound session |
...
`allowed_models` and `reasoning_effort` are the same shape of field as `llm` in every way that matters: baybo-framework-only (stored regardless of `framework`, greyed out for external frameworks in the editor), locked on the builtin profile like every other content field, and read live at actor spawn rather than snapshotted at bind time. `allowed_models` is a de-duplicated (order-preserved) set of `baybo.json` entry names — it constrains which model an explicit session switch may *choose*, never the resolved default, so it has no effect on `resolve_spawn_llm`'s own precedence (below) or on a session that never switches. `reasoning_effort` is a `ReasoningEffort` value (`Minimal | Low | Medium | High | XHigh`; a new `baybo-model` unit enum with the house `as_str()` / `parse()` / `const ALL` mirror) carried to the LLM layer per request — see [`llm.md`](llm.md#reasoning-effort-bridge) for the delivery mechanism and provider consumption. Both fields are validated at write and tolerated at read, matching the `llm` pin's discipline — see [Design Decisions](#the-llm-pin-is-validated-at-write-tolerated-at-read) below.
...
- **LLM pin.** `resolve_spawn_llm` (`crates/agent/src/actor/router/user_input.rs`) is the precedence: the session's explicit `last_llm` pin wins for the *model* if set; otherwise the bound profile's `llm` is read live from the store; otherwise the pool default. This resolves **at actor spawn / hydration**, not per turn — a profile edit lands the next time the actor is (re)built (cold start, or after an idle reap), while an explicit per-session model switch (`PUT /v1/chat/sessions/{id}/model`) always wins immediately via the existing `AgentMessage::SetModel` re-pin. `resolve_spawn_llm` returns a `SpawnLlmChoice { initial_llm, reasoning_effort }`; it fetches the bound profile even when `last_llm` is already set, because the pin only ever supplies the model — `reasoning_effort` (below) comes solely from the profile, so it needs the live read regardless.

  **Allowed-models set.** When the bound profile's `allowed_models` is non-empty, it constrains what `PUT /v1/chat/sessions/{id}/model` may switch *to* — the resolution precedence above is untouched by it. After the existing `validate_llm_pin`, a request that supplies a **new** pin (not a clear) re-reads the bound profile fresh and 400s ("model {name} is not in agent {agent}'s allowed set") if the name is outside the set; `llm: null` (clearing the pin, reverting to `default-llm`) always bypasses the check — the set constrains explicit choices, not the system default; a profile that has since been deleted skips the check entirely (the same tolerance as every other live read — see [Deletion tolerance](#deletion-tolerance)) rather than blocking the switch; a store error on that lookup surfaces as 500. A historical pin that ends up outside a later-shrunk set (the profile was edited after the switch) is tolerated the same way a stale `llm` is — see [Design Decisions](#the-llm-pin-is-validated-at-write-tolerated-at-read) — except that `resolve_spawn_llm` does read `allowed_models` in this one case, purely to emit a `warn!` at spawn; it never rewrites or rejects the pin.
- **Reasoning effort.** Read by the same `resolve_spawn_llm` call as the LLM pin, the profile's `reasoning_effort` lands on `AgentLoopConfig.reasoning_effort` and is filled into `ChatRequest.reasoning_effort` **only** by the main loop's `AgentLoop::call_llm` — title generation, the progress observer, and compression side-calls always send `None`, regardless of the profile, so they stay cheap and provider-default. Liveness matches the `llm` pin: a profile edit lands on the next actor spawn/hydration, not mid-session. Unlike the `llm` pin, there is **no** live re-pin path for it — `AgentMessage::SetModel` only ever carries a model name, never an effort, so an in-flight actor keeps its spawn-time effort until it is next (re)built. See [`llm.md`](llm.md#reasoning-effort-bridge) for how the value is carried to the provider and which providers consume it.
...
`allowed_models` and `reasoning_effort` follow the identical write-time discipline: `allowed_models` is validated against the live pool member-by-member (`validate_llm_pin` per name, 400 otherwise), de-duplicated with order preserved (a normalization, not an error), and, when `llm` is also set and the set is non-empty, checked that the pin is a member (400 otherwise); `reasoning_effort` must parse via `ReasoningEffort::parse` (400 listing the legal values otherwise). Both share a helper beside `validate_llm_pin` in `crates/gateway/src/api/admin/agents.rs`. Staleness is tolerated the same way as `llm` at the *switch* consumption site: a set member later removed from `baybo.json` renders "(unavailable)" in the editor and simply never matches (see the allowed-models switch enforcement above). At the storage *read* boundary, though, `allowed_models` and `reasoning_effort` are stricter than `llm`, matching the `framework` column's rule rather than the pin's: a malformed `allowed_models` JSON blob or an unrecognized `reasoning_effort` string is a hard read error, never a silent fallback — write validation is what keeps either from occurring in practice, and only an out-of-band DB edit could trigger it.
...
CREATE TABLE IF NOT EXISTS agent_profiles (
    ...
    allowed_models  TEXT,                   -- JSON array of LLM entry names; NULL = unrestricted
    reasoning_effort TEXT                   -- ReasoningEffort::as_str(); NULL = follow the entry's value
);
```

`allowed_models` / `reasoning_effort` are guarded `ALTER TABLE … ADD COLUMN` in `init_db()` (both `TEXT`, defaulting `NULL`), landing after the original column set so an existing DB migrates in place without a data rewrite. An empty `allowed_models` set is stored as `NULL`, never `"[]"` — the same "NULL and empty round-trip to the same unrestricted state" rule `sessions.hidden`-adjacent flat columns follow elsewhere.

House encodings throughout: timestamps µs via `super::time`, booleans `INTEGER` 0/1 (an unknown `framework` string on read is an error, never a silent fallback — `allowed_models`/`reasoning_effort` read errors follow the same rule, see above). No `skills` column — skills are read live from the registry, not stored per agent.
...
| `POST /v1/agents` | 200 `AgentProfileDto` | 400 name invalid/duplicate, unknown `llm`, unknown `allowed_models` entry, `llm` ∉ `allowed_models`, unknown `reasoning_effort`, unknown/non-image `avatar_blob_id`; 401 |
| `GET /v1/agents/{agent_id}` | 200 `AgentProfileDto` | 401, 404 |
| `PUT /v1/agents/{agent_id}` | 204 | 400 builtin locked, name invalid/duplicate, unknown `llm`, unknown `allowed_models` entry, `llm` ∉ `allowed_models`, unknown `reasoning_effort`; 401, 404 |
...
- **Detail**: an inline character-sheet editor (no modal, centered single column), keyed by selection. Header: large avatar portrait + image/remove controls beside the name field, a fixed-min-height meta row (builtin lock badge / id) so switching agents never jumps the layout. Body: description, framework, LLM pin, system prompt, then — below the pin — an **allowed-models checkbox list** (one row per `GET /v1/llm/models` entry; none checked = unrestricted, matching the empty-set semantics) and a **reasoning-effort select** ("Default (entry setting)" plus the five `ReasoningEffort` values), then a full-width **read-only skills readout** — every skill visible to that agent listed with its `SKILL.md` description (`GET /v1/skills` unscoped for the builtin/no-selection/create-new state, `?agent_id=<id>` scoped to that profile's `agent-skills/` overlay otherwise — mirrors `SkillRegistry::summaries_for_agent`, see [`skills.md`](skills.md#agent-scoped-overlay)). A removed LLM entry renders as "(unavailable)" both in the pin select and, per stale member, in the allowed-models checkbox list (still shown, still removable, just not selectable as a *new* member); the LLM pin, the checkbox list, and the effort select all grey out together for external frameworks ("baybo only"). When the checkbox list has at least one member checked, the LLM pin `<select>` narrows to just those members — a client-side mirror of the server's write-time "pin ∈ set" rule, not a substitute for it. Footer: destructive Delete on the left (custom agents; a confirm dialog follows), Save on the right. For the builtin everything except the avatar is disabled and Save is live only when the avatar changed.
...
- **ModelPicker filtering** (`app/web/src/pages/ChatPage.tsx`, `filterAllowedModels` in `app/web/src/pages/chat/modelFilter.ts`): for a session bound to an agent whose `allowed_models` is non-empty, the model-switch picker is filtered to just those entries — an empty set is treated as "unrestricted" and leaves every configured model visible, mirroring the server's own semantics. If the session's currently-effective model falls outside the *filtered* list, the picker still shows it rather than hiding it, flagged rather than silently vanishing — the sublabel distinguishes why: "(not in allowed set)" when the model is still a live pool entry the agent's set just excludes, "(unavailable)" when it's gone from the pool entirely (removed from `baybo.json`). This flag only ever fires for an **explicit** pin (`current` non-null): a session left on "Default" is never flagged even if `default-llm` itself resolves outside the set — the set constrains explicit choices, not the default, and clearing a pin always bypasses it (see the allowed-models switch enforcement above). The agent's `allowed_models` rides the same `GET /v1/agents` fetch the picker and chips already use — no new endpoint, no new WS frame.
...
| Module | Role |
|---|---|
| `model` | `AgentProfileId` (ULID-minted string newtype), `AgentFramework` (+ `to_backend_kind`), `BUILTIN_AGENT_PROFILE_ID`, `MAX_AGENT_PROFILE_NAME_CHARS`, `ReasoningEffort`; `SessionState.{agent_id, agent_framework}` + `agent_id_or_builtin()` |
| `store` | `AgentProfileRow` / `AgentProfileUpdate` / `AgentProfileStore` port (incl. `allowed_models` / `reasoning_effort`); `StorageError::Conflict` carries duplicate names |
| `storage` | `LibsqlAgentProfileStore` (async `open` seeds the builtin), `agent_profiles` DDL + the `allowed_models`/`reasoning_effort` guarded `ALTER TABLE`s in `init_db()`, `Store.agent_profile` bundle field; `sessions.{agent_id, agent_framework}` columns seeded by `save`'s INSERT, immutable thereafter (no setter) |
| `gateway` | `api/admin/agents.rs` handlers + DTOs, `AdminState.{agent_profile_store, blob_store}`, the shared `validate_llm_pin` + `validate_allowed_models` + `validate_reasoning_effort`; `api/admin/chat.rs` `resolve_agent_binding` at session creation and `set_session_model`'s allowed-set enforcement; `GET /v1/skills` (`?agent_id=` scoped) and `GET /v1/llm/models` feed the read-only skills readout and the model picker |
...
| `agent` | the `LlmPoolHandle` on `AdminState` validates the `llm` pin at write time; `resolve_spawn_llm` (session pin > live profile pin > default, plus the live profile's `reasoning_effort`) at actor spawn/hydration; `AgentLoopConfig.reasoning_effort` → main-loop-only `ChatRequest` fill; `ToolContext.agent_id` threading |
| `llm` | consumes `ChatRequest.reasoning_effort` — see [`llm.md`](llm.md#reasoning-effort-bridge) for the `additional_params` bridge and the provider consumption matrix |
...
| `web` | `AgentsPage.tsx` (management + scoped skills readout + allowed-models checkbox list + effort select), `AgentPicker.tsx` + session chips (chat consumption), `ChatPage.tsx`'s `filterAllowedModels`-driven ModelPicker filtering, route + `IconRail` entry, blob upload/render reuse |
