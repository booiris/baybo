...
| `storage` | `SqliteAgentProfileStore` (async `open` seeds the builtin), `agent_profiles` DDL + the `allowed_models`/`reasoning_effort` guarded `ALTER TABLE`s in `init_db()`, `Store.agent_profile` bundle field; `sessions.{agent_id, agent_framework}` columns seeded by `save`'s INSERT, immutable thereafter (no setter) |
