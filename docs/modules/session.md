...
- `agent_id` / `agent_framework`: agent-profile binding — which `agent_profiles` row (if any) this session was created under, and a snapshot of that profile's framework at creation. `None` on both = the builtin `baybo` agent (`SessionState::agent_id_or_builtin()` normalizes this to `BUILTIN_AGENT_PROFILE_ID`). Seeded onto `SessionState` before the session's first `save()`, so it rides that INSERT into the flat `sessions.agent_id` / `agent_framework` columns (the `hidden` / `pinned` pattern) — `save`'s upsert never updates those columns and no setter exists, so the binding is structurally immutable for the session's life, not just guard-enforced. See [`agent-profiles.md`](agent-profiles.md#session-binding).
