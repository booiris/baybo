...
comes from the caller's `user_id` at every call; `agent_id` defaults to
`"baybo"` (deployment identity) on writes and is overridable per tool call.
Tool reads accept an optional `scope: "session"` that narrows to the current
session via Mem0's `run_id` (sourced from `ToolContext::session_id`).
