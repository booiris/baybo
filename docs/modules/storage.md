...
Fields difficult to fully structure (`SessionState.extra`, `Job.input/output`) are stored as JSON. Trace tables use a columnar schema: `session_traces` holds only metadata (root, active_leaf, timestamps); `trace_nodes` stores one row per node with queryable columns (`kind`, `started_at`, `parent_id`, `span_id`, `span_index`, `span_role`, etc.) and JSON for complex fields (`provenance`, `input`, `result`, `snapshot`). Children lists are derived from `parent_id` on load. There is no forks side-table — in-session branches are reconstructible from the parent/child pointers; cross-session forks live on `Session.parent_link`. The security requirement still applies: values must already be sanitized.
