...
A second, unrelated cursor sits beside it under the same discipline: `dreamed_through_ordinal` is how far the built-in memory pass has been offered this conversation, advanced max-wins by `SessionStore::set_dreamed_through_ordinal` after a dream fire is dispatched. It is deliberately an ordinal rather than a timestamp — the rows a pass misses are the ones appended *after* it read, which are the agent's own, so no time-and-human-message window can describe them. See [memory-builtin.md](memory-builtin.md).
