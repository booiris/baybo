...
1. **Hot-updatable whitelist** — explicit allowlist of fields safe to swap live. Plausible: `cost.rate_limit.*`, `cost.spending_limits.*`, `security.leak_detection_enabled`. Explicitly not hot-updatable: `channels.http.port`, `channels.http.bind_address`, anything that influences `LlmClient` identity.
