...
- **Left, 120px `JobSidebar`** — a flat list of turns (`#1 #2 #3…`) carrying only
...
- **Right, 480px** — `SpanDetailPanel` (I/O · Meta · Events) for a selected span, or
  `TurnSummaryPanel` when nothing is selected.

Three pain points surface once a session gets complex:

1. **Turn navigation doesn't scale.** The narrow flat `#N` list gives no way to tell
   *which* turn holds the problem, and no structure to drill through.
2. **Low information density.** The heavy step cards mean only a few steps fit on
   screen; a turn with dozens of steps is all scrolling.
...
│ ▸ Turn #1  ok  12s          │  span → I/O·Meta·Evt  │
│ ▾ Turn #2  ● 1 FAIL         │  Step → outcome +     │
│    ▸ LLM iter ok            │         failure + span│
│    ▾ LLM iter ● FAIL        │  Turn → in/out/token  │
│       ▸ Tool bash ● FAIL ◀  │  subagent → child     │
│    ▾ ⧉ Subagent→child ●FAIL │      overview + Task  │
│       ▾ [child] Turn#1 ●FAIL│      prompt + open    │
│ ▸ Turn #3  ● 2 FAIL         │  ┌ breadcrumb ─────┐  │
│ [全部|仅失败|按kind|搜索…]  │  root›#2›iter›bash    │
└─────────────────────────────┴──────────────────────┘
```

**Hierarchy.** `Session → Turn → Step → Span`, matching `docs/modules/trace.md`
(`Session > Turn > Step > Span (+ SpanEvent)`, a fixed three-layer fan-out; steps and
...
- On load, collapse everything to turn level, but **auto-expand the ancestor chain of
...
- A **filter bar** at the top of the tree: a text search (kind / tool / model) + a
  **Failures only** toggle. When any filter is active, every turn's step tree is
  eager-loaded so the filter never silently hides a matching-but-unloaded turn.
...
- One up-front call fetches the whole lineage tree + each descendant session's turn
...
- The heavy per-turn step/span tree stays **lazy** — fetched only when a specific step
  region is expanded.
- A child session renders under its spawning span with a session-boundary marker
  (`⧉ Subagent → child …`), indented; its own `Turn → Step → Span` nests beneath.
...
- Every node is selectable; the right panel is contextual:
  ...
  - **Step** → a new step panel: outcome + failure reason + a summary of its spans. A
    ...
    that span's detail.
  - **Turn / child session** → existing `TurnSummaryPanel` (input / output / tokens /
    activity).
  - **subagent container** → child turn overview + the spawn context (the `Task`/spawn
    tool call's prompt & params) + an "open child as full page" link (keeps today's
    navigate as a secondary action).
- The right-panel header carries a **breadcrumb** (`root › turn › … › child › node`) so
...
- The step tree of each **currently-expanded and live** turn on the fast tier (~2s);
  collapsed turns don't poll. Visibility-aware (existing `document.visibilityState`
  gate).
...
- `baybo_model::Lineage`: `{ parent_session_id, parent_turn_id, parent_span_id,
  kind: Subagent }` — `parent_span_id` is the parent's `ToolCall(spawn_subagent)` span,
  the exact attach point, and disambiguates sibling subagents from one turn.
...
(`GET /v1/traces/{session_id}` overview + `/turns/{turn_id}` step tree). Subagent
...
- New tree components (`app/web/src/components/trace/…`): a recursive tree row + the
  Turn / Step / Span node renderers, expand-state store, roll-up badge, filter bar.
...
- On load, eagerly fetch the step trees of turns whose `turn_status_kind` is
  `failed`/`stuck` (to draw the failure path); other turns stay lazy on expand.
- Right panel: reuse `SpanDetailPanel` + `TurnSummaryPanel`; add the **Step panel** and
  the breadcrumb header.
- URL: keep `?turn` / `?span` / `?tab`; add a `?step` selection.
- Update `app/web/src/api/mock.ts` fixtures to include a **failed** turn/step/span so the
...
default `true`; `neededTurnIds` loads every non-user-collapsed turn — nothing is hidden),
...
step/span across all loaded turns, kind-coloured via a literal `cell` field, red for
...
is the key). Plus a left **turn column** (`TurnAnchors`) — a document-outline
list that selects + scrolls the tree to a turn (`data-turn-id`), previewing the **user input**
that started each turn (so turns are told apart by what was asked, not by number) along with
its tokens, duration, and a red dot when it holds a failure. Hidden for single-turn traces. Still not ported: per-step **ordinals** and
...
- **Collapsed-turn failure badge without a loaded trace** uses `turn_status_kind` as a
  cheap approximation (a `failed`/`stuck` turn → badge). Once the tree is loaded the
  roll-up counts failing leaves exactly, but still keeps the badge for a `stuck`/turn-
  level failure whose spans are all `ok`. Acceptable for an admin/debug surface.
- **Filtering eager-loads every turn's step tree** (only while a filter is active) so
