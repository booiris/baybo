...
10. ~~**`security audit` / `leaks check`**~~ — shipped. `audit` reports rule count by action + vault master-key flag (never secret material). `leaks check` reads a file on disk and reports blocked/hits via the shared `LeakDetector`. **`skills search/check`** still pending.
