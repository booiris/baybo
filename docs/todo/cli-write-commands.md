...
11. ~~**`agent send`** — depends on Router's life cycle (the one-shot call must share the chat loop's spawner).~~ — partial. CLI grammar (`--session`, `--message`, `--yes`) and the `AgentSendForbiddenInSlash` slash-mode guard are shipped; argv mode currently returns a clear `Manager` error ("not yet available in argv mode") because `Router` does not yet expose a synchronous one-shot entry. Wire the real path when that entry exists or a daemon RPC surface lands.
