...
| `MemoryManager`        | `list(user_id)`, `search(user_id, query)`, `set_importance(id, importance)`, `delete_for_session(id)`                                                                                                                                                | `memory list/search/promote/clear`                                    |
...
5. **`trace`** — touches file I/O for `export`; snapshot is a write through an existing hook.
