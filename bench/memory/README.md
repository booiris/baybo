# baybo-bench-memory — memory benchmark

Measures how much each memory backend (`mem0` / `openviking`) helps **the real
Baybo agent** answer questions about a long, multi-session conversation it can no
longer see in context — the analog of how upstream's OpenViking benchmark drives
OpenClaw. The answering agent runs the full Baybo loop (recall + memory tools);
judging runs on DeepSeek (`--judge-model`, default `deepseek-v4-pro`).

The bench drives the agent as a **black box**: it execs the `baybo` binary
(`baybo gateway` once per arm, then a concurrent `baybo prompt` per question over
it) — no linkage to Baybo's agent stack. Its one library dependency is
`baybo-memory`, used by the `ingest` bin to populate the backend directly.

> ⚠️ **Not a CI / `cargo test` target.** The bins start a real `baybo` gateway and
...
2. **`run`** — for one arm: build a gateway config, start `baybo gateway` against
   it, then answer each question with a concurrent `baybo prompt --json --session
   ...
   `baybo gateway start` mints its own token, so nothing pre-exists. Pass
   `--baybo-config` to derive from your own `baybo.json` instead (reuse its
   workspace/keys/llm, overwrite only the memory section). Each `baybo prompt`
   ...
   answer-side token counts, and cost (read back per question from Baybo's cost
...
| --- | --- | --- |
| `--baybo-config` | `run` | **optional override.** Omit → self-contained config (fresh workspace + minted key + the configurable answer model below). Given → derive from yours (reuse its workspace/keys/llm), overwriting only the `memory` section. |
| `--answer-model` / `--answer-provider` / `--answer-api-key-env` / `--answer-base-url` (env: `BAYBO_ANSWER_MODEL` / `BAYBO_ANSWER_PROVIDER` / `BAYBO_ANSWER_API_KEY_ENV` / `BAYBO_ANSWER_BASE_URL`) | `run` (self-contained only) | the model Baybo answers with. Defaults reproduce the DeepSeek answerer (`deepseek` / `deepseek-chat` / `DEEPSEEK_API_KEY`, provider default endpoint); point them at any Baybo-supported provider to benchmark its cost/quality. Ignored with `--baybo-config`. |
| `--baybo-bin` | `run` | the `baybo` built with `--features bench-readonly-memory` (default `baybo` on PATH). |
| `DEEPSEEK_API_KEY` | `run` | judge model always; the self-contained answer model when `BAYBO_ANSWER_API_KEY_ENV` is left at its default — the gateway inherits whichever key env you point it at |
...
So you write **no `baybo.json`** and need no pre-existing `~/.baybo` — the bench
...
model** (the model Baybo answers with) defaults to DeepSeek `deepseek-chat`, but
...
the comparison direct). With `--baybo-config` the answer model is instead that
config's `default-llm` and QA sessions land in its workspace (harmless — memory
writes are off under the feature). No other `baybo gateway` may run in the
workspace in use.
...
Easiest — `run.sh` builds the read-only `baybo`, auto-loads `.env`, ingests
...
cargo run -p baybo-bench-memory --bin ingest -- --arm openviking --dataset locomo10.json
cargo run -p baybo-bench-memory --bin run -- --arm openviking --dataset locomo10.json \
    --baybo-bin ./target/release/baybo --manifest manifest-openviking-<run_id>.json

# Floor / ceiling — memory off, no ingest:
cargo run -p baybo-bench-memory --bin run -- --arm noop   --dataset locomo10.json --baybo-bin ./target/release/baybo
cargo run -p baybo-bench-memory --bin run -- --arm oracle --dataset locomo10.json --baybo-bin ./target/release/baybo

# Plan / cost without starting anything (no config or keys needed):
cargo run -p baybo-bench-memory --bin run -- --arm openviking --dataset locomo10.json --dry-run
```
...
`trace/<run_id>/<arm>/<session>.{messages,trace}.json` (`baybo session history` +
`session export`) — the raw record of what baybo recalled, answered, and called per
...
**`trace_recall.py` — what baybo actually recalled.** Reads each question's exported
...
question + gold + correct, every recalled block, and baybo's final answer — pinning
...
```bash
# wrong answers in conv 0 and the memory baybo saw for each
...
- **Read-only build is mandatory.** Without `--features bench-readonly-memory`,
  ...
  bench can't detect that you forgot — make sure `--baybo-bin` is the feature
  build.
...
- **Answer-time, tokens, and cost are surfaced** per question, sourced from
  Baybo's `cost_records` ledger via a per-question `baybo cost show --session <id>
  ...
  time. In `--baybo-config` mode these rows land in your real workspace ledger
  under the bench's `qa-*` sessions and are left in place (Baybo never
  auto-deletes cost history), so they also surface in untargeted `baybo cost`
  totals.
