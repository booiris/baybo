...
```bash
bench/swe-baseline/view_compare.sh          # latest aura + mini runs → serve on :8091
...
AURA_RID=full300-… MINI_RID=baseline-full300-… bench/swe-baseline/view_compare.sh
```
...
```bash
# aura: trace .messages.json -> trace/<RID>/_export/<id>.md + index.md
python ../swe/export_aura_trajs.py --run-id <AURA_RID>
...
python compare_arms.py \
  --aura-export ../swe/trace/<AURA_RID>/_export \
  --mini-export runs/<MINI_RID>/_export \
  --aura-results ../swe/results/results-agent-<AURA_RID>.json \
