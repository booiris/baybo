...
|---|---|---|---|
| 1 | Markdown fidelity for description / comments | **Two kept-warm WKWebViews** running `issue.html` (the third Vite entry). The whole issue body goes into the webview; native keeps only the header and the composer dock | §3.3 |
...
- Shape: a third Vite entry `issue.html`; an `IssueBridge` (the size of `DeckBridge`, with its main-frame-only guard and 3-reloads-per-30s crash budget): native→web `init / deliver / setBottomInset / jumpToLatest / blobResult`, web→native `issueReady / issueRendered / pick(field) / openRun / openIssue(n) / activityAtBottom / issueState / requestBlob / viewImage / previewFile / copy / log`. **Inline images ride the bridge's `requestBlob`/`blobResult` (as the transcript does), so no scheme route is needed**; putting HtmlPreview in the issue body would need `DynamicRoute` widened, which v1 skips. The keyboard: the webview never resizes; native streams the bottom inset (the transcript's mechanism).
- **The renderer is a two-slot warm pool, not one cold webview per card.** `AppStore.issueHostPool` constructs both `IssueHost`s when a binding reaches Home and keeps them until logout/rebind; an idle memory warning may discard both. Two is the minimum for a native transition because source and destination are simultaneously visible. Every navigation entry carries a fresh `ProjectIssueRoute.id`; its `IssueVisit` owns the card's `IssueStore`, draft and page state, while its `Lease` names one renderer slot. `openProjectIssue` creates and retargets the visit before appending the route, so the destination's already-loaded React runtime has the board/mirror payload before the push starts.

  The allocator alternates slots by stack depth. With A → B → C, A/C use slot 0 and B uses slot 1; C may replace A because only adjacent pages can be visible. Once C → B finishes, `IssueDidAppearReporter` restores A into slot 0 while B covers it, so the next interactive B → A pop has two live surfaces again. This repeats at arbitrary depth. Reparenting is claim-time and identity-guarded; a late dismantle from C cannot detach the host already restored to A.

  Retargeting is also a state boundary. Each visit keeps `scrollTop`, activity-fold choices, bottom inset and composer geometry in memory. One JavaScript evaluation snapshots the outgoing page and initializes the incoming `targetId` in that order; `issue/bridge.ts` clears the old one-holder subscription before buffering the new card's delivery. The reused root stays hidden until the bridge has both the new init and its first payload; React then commits that pair under the new key and reveals it in the same frame, so no old-card → `Loading card` → new-card intermediate tree paints. Every issue-specific async callback carries the id, so a generated avatar, delayed scroll report or retry from the covered card cannot mutate the new store.
- **`.onDisappear` is still not exit.** A push fires it too, so only the reversible paste-target handover lives there. Popping a route releases its `IssueVisit`; `IssueStore.deinit` retires the staging machine, while the host returns warm to the pool. The bridge holds no closures the screen writes: `pick`, `openRun` and `activityAtBottom` land as store state (`pickRequest` / `openRunRequest` / `atBottom`), and `IssueBridge.store` is weak. That is what lets a renderer outlive many cards without retaining any of them.
...
- REST marks reconstructed work rows closed, even when a run has no final assistant output. The iOS reader therefore defaults the trailing work sequence open in that case; every adjacent half split by a compaction watermark opens around the `Compacted` divider. A final assistant output keeps the ordinary collapsed `Worked` default.
...
- **Activity (pushed)**: a read-only stream; a row is a glyph, one sentence (actor / #N / the word that ends a run in ink) and a relative time; numbered execution events say `turn N`, reserving `#N` for cards; failure is the only red; tapping opens the card. Unknown timeline/feed kinds must render through a fallback — master is still adding kinds.
...
- Pure logic lives in `Core/`: `BoardOrder` (pinned→unread→updated desc, with position/number ties), `MoveConsequence` (the Move sheet's sentences), `CommentHint`, `PendingApprovals` (replay by `call_id`), `BudgetMeter`, `RunLabels` (including elapsed).
...
A first-ever card has no such file. If its board is already in memory,
`ProjectsStore.issueSeed` supplies that board's issue row, runs, team and
children to the new `IssueStore`; the warm page can therefore paint the title,
description and state before the card's five reads return. A card mirror wins
over this seed when both exist. The seed is marked with the same
`isFromMirror` gate because it is still an unconfirmed snapshot: it may draw,
but it may not offer Stop or a parked approval until the card endpoint itself
answers.

Scroll position and opened activity folds are a different cache: they belong
to one `IssueVisit`, stay only in memory, and survive while deeper child cards
cover it. They are not written into the content mirror and do not survive a
pop or process restart. Rebuild clears them together with the page document.
