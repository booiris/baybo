...
### The draft belongs to the SESSION, not to the composer frame

`ComposerStaging` is owned by `ChatStore` (`store.staging`) and holds **the whole unsent draft** — the typed text as well as the strip. `ComposerView` keeps no `@State` of its own for either.
