...
The viewer's top-right button opens the system share sheet on the blob materialised under its real name (`writePreviewFile`, shared with the file path) — the FILE, not the decoded `UIImage`, so Save-to-Photos / Files / AirDrop keep the encoding that was uploaded — the original, except where staging normalised it: a HEIC from Photos or paste left the phone as a JPEG and a TIFF-only paste as a PNG (§ *The mime decides everything downstream*; a Files pick keeps its HEIC).
...
- **A HEIC leaves as JPEG** (`StagedAttachment.uploadablePhoto`, inside `spoolPhoto` — the funnel the photo picker and paste share, off the main actor). HEIC is what a camera-roll pick is, and Anthropic's and OpenAI's APIs refuse it: the gateway delivers png / jpeg / webp only (`is_portable_image_type` in `crates/llm`), so the photo reached the agent as a `[image: image/heic …]` placeholder — and before that gate existed, one such photo failed every later turn of an Anthropic session. The re-encode is ImageIO's thumbnail decode (orientation drawn into the pixels) into a JPEG at quality 0.8, long edge at most **4096 px** — a 12 MP photo keeps its size, and 4096 is the edge `IMAGE_TOKEN_CEILING` is priced from, so it reaches every provider — retried at 2048 px when a detailed scene would still exceed the 5 MiB `MAX_IMAGE_DOCUMENT_BYTES` delivery cap. The mime, the spool's extension and the size the card reports are all the JPEG's; the metadata is not carried over, so a photo's location no longer rides to the provider. This is the rule above working, not an exception to it: what the paste bullet forbids is an unconditional decode into a far larger PNG under a mime that no longer describes the bytes, and this re-encode is smaller and re-names what it produced. A HEIC ImageIO can't decode fails the thumbnail decode that follows as well, so the pick is dropped with *attach failed*, like any photo ImageIO can't read. **A Files pick is not re-encoded** — it uploads in place with no spool (see *A Files URL is security-scoped* and *The spool is owned, not deleted*), so a `.heic` from Files still reaches the model as that placeholder.
...
**Every** staged pick uploads with `blob_upload_file(path, mime, progress)` — the bytes never cross the FFI, so a 100 MiB attachment is never held whole in memory. A Files pick already has a path; a photo does not (PhotosUI hands over a `Data`), so its bytes — for a HEIC, its JPEG rendition — are **spooled to `<tmp>/baybo-compose/<run id>/<pick id>.<ext>`** at staging and the tile keeps only that file. A pasted image has no path either, and takes that photo branch verbatim — `stagePasteboard` reduces to `admitPhoto()` + `acceptPhoto(id:data:declaredMime:)`, so the spool, the `SpoolFile` ownership, the downsampled thumbnail, the byte cap and `pumpUploads` are all inherited rather than re-derived. Holding the `Data` on the item instead — which is what a retry would need — is ten full-size encoded picks alive for as long as the strip is up, on top of ten decoded thumbnails: a foreground jetsam on realistic ProRAW/panorama picks. The thumbnail is decoded **downsampled through ImageIO** off the spooled file (`CGImageSourceCreateThumbnailAtIndex`, 256px) rather than with `UIImage(data:)`, which would keep the encoded bytes alive behind the image.
...
**A card comment owns a small durable outbox.** `IssueStore.sendComment` persists the text and the full attachment metadata before the dock clears the draft. The timeline paints that row immediately, delays its spinner by the same 900 ms as Chat, and changes it to an inline retry on failure. Retry reuses the original `client_msg_id`, so the gateway's idempotent append cannot duplicate a comment whose first response was lost.
