...
**Push registration bounds.** The device-token store is bounded: a soft cap (**65 536** bindings, override `PUSH_DEVICE_STORE_CAP`) with eviction of idle, never-`/notify`-confirmed entries (TTL **1 h**, override `PUSH_UNCONFIRMED_TTL_SECS`), so a `/register` flood of valid self-signed chains can't grow it without bound — at the cap with nothing evictable a new `/register` is shed `503`. A `/register` whose provider token is empty or exceeds the cross-provider cap (**4096 bytes**) is refused `400` before it can bloat a stored binding; a bogus-but-bounded token is caught when its provider permanently rejects it and the binding is pruned. A target whose provider is not configured is refused `503`. The notify-limiter map itself is bounded against an id-churn by a fixed soft cap (**16 384**).
...
- **Logs** roll daily into `LOG_DIR` (default `/data/logs` → host `./data/logs`, on the `./data` volume so they survive `up --build`), **14 daily files** retained, and are mirrored to stderr so `docker compose logs -f` keeps working (that Docker copy is capped at `10m × 5` files). The hot path is silent by design — no per-message / per-byte / per-connection logging — and the default level narrates anomalies, not each successful session or push: admission rejects (unknown/expired key, with the key's redacted first-8 tag + client IP, debounced per key), gateway control connect/disconnect, `content-join` refusals (gateway not connected — debounced across the phone's retries — / control backpressure / owner mismatch), the unclaimed-leg smoking gun (gateway signaled but never dialed back), per-IP/per-key/parked-leg limit backstops, admission reload diffs + revoke kicks, and the `/register`+`/notify` reject/prune pipeline (register success, each verify-step reject with replay counters, APNs verdicts incl. token prunes); per-splice/per-close leg detail and each delivered push (with its `apns-id`) log at `remote_host_relay=debug` / `remote_host_push=debug`. Credentials never appear raw: `remote_api_key`/`relay_key` log as first-8 tags, APNs tokens as lengths; `relay_node_id`/`device_id` are routing handles, **not** secrets. Tune verbosity with `RUST_LOG` (blank → the quiet default `warn,remote_host=info,remote_host_relay=info,remote_host_push=info,remote_host_edge=info,remote_host_admission=info,remote_host_dashboard=info`): `RUST_LOG=warn` for problems-only, append `remote_host_relay=debug` to trace a gateway.
