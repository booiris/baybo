 ...
 * (`fetchBlob`). Auth reuses the same `BAYBO_CHANNEL_TOKEN` the
 ...
 * Pass an explicit `baseUrl` to override, or set `BAYBO_CHANNEL_URL`
...
export interface BlobClientOptions {
  /** Override the gateway base URL. Defaults to `BAYBO_CHANNEL_URL` derived. */
  baseUrl?: string;
  /** Override the bearer token. Defaults to `BAYBO_CHANNEL_TOKEN`. */
  token?: string;
   ...
   * blob writes. The SDK forwards it as the `x-baybo-bot-id` header.
   */
  botId?: string;
  /**
   * Originating platform user id. Same role as `botId` — needed by
   * the pairing gate. Forwarded as `x-baybo-user-id`.
   */
  userId?: string;
}
 ...
 * `baybo pair approve <code>` to authorize the binding; sidecars
...
export async function uploadBlob(
  ...
  const headers: Record<string, string> = {
    ...
    "x-baybo-channel-token": token,
  };
  if (opts.botId) headers["x-baybo-bot-id"] = opts.botId;
  if (opts.userId) headers["x-baybo-user-id"] = opts.userId;
...
export async function fetchBlob(
  ...
  const res = await fetchWithSignal(url, {
    method: "GET",
    headers: { "x-baybo-channel-token": token },
    ...(opts.signal ? { signal: opts.signal } : {}),
  });
  if (!res.ok) {
...
function resolveAuth(opts: BlobClientOptions): { baseUrl: string; token: string } {
  ...
  if (!token) {
    throw new Error(
      "blobs: missing token — pass opts.token or set BAYBO_CHANNEL_TOKEN",
    );
  }
 ...
 * `BAYBO_CHANNEL_URL` is set by the gateway's `ChannelSpawner` to
 * `ws://127.0.0.1:<port>/v1/channel-ws`, so this is a literal scheme
 * swap with the path stripped.
 */
function deriveBaseUrl(): string {
  const wsUrl = process.env["BAYBO_CHANNEL_URL"];
  if (!wsUrl) {
    throw new Error(
      "blobs: missing baseUrl — pass opts.baseUrl or set BAYBO_CHANNEL_URL",
    );
  }
  let parsed: URL;
  try {
    ...
    throw new Error(`blobs: invalid BAYBO_CHANNEL_URL ${wsUrl}: ${String(e)}`);
  }
  const httpProto =
