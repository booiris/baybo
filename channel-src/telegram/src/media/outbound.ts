 ...
 * based on the wire-level `kind` and the underlying MIME type. The
 * caller has already pulled the bytes via `fetchBlob`. `caption` is
 * applied to this attachment only — the platform layer rotates it
...
export async function sendTelegramAttachment(
  ...
  const file = new InputFile(bytes, inputName);
