import { getConfig } from "./endpoints.js";
...
/**
 * Per-user getConfig cache with random-jittered TTL (≤24h) and
 * exponential-backoff retry (up to 1h) on failure. Typing indicator is
 * best-effort — a cache miss never blocks message processing.
 */
    ...
    if (shouldFetch) {
      ...
      try {
        const resp = await getConfig({
          ...
          ...(this.apiOpts.token !== undefined ? { token: this.apiOpts.token } : {}),
          ilinkUserId: userId,
